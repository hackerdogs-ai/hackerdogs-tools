"""
Test suite for EmailFormat tool (emailformat)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emailformat_langchain import sfp_emailformat
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emailformat_crewai import SfpEmailformatTool
...
class TestEmailformatStandalone:
    """Test EmailFormat tool standalone execution."""
    
    def test_emailformat_standalone(self):
        """Test emailformat tool execution without agent."""
        ...
        result = sfp_emailformat.invoke(invoke_params)
        ...
        result_file = save_test_result("emailformat", "standalone", result_data, target_filename)
...
class TestEmailformatLangChain:
    """Test EmailFormat tool with LangChain agent."""
    
    def test_emailformat_langchain_agent(self):
        """Test emailformat tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_emailformat]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the EmailFormat tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use EmailFormat to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("emailformat", "langchain", result_data, target_filename)
...
class TestEmailformatCrewAI:
    """Test EmailFormat tool with CrewAI agent."""
    
    def test_emailformat_crewai_agent(self):
        """Test emailformat tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using EmailFormat",
        ...
        task = Task(
            description=f"Use EmailFormat to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from EmailFormat tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("emailformat", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running EmailFormat Tool Tests (emailformat)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestEmailformatStandalone()
        test.test_emailformat_standalone()
    ...
    try:
        test = TestEmailformatLangChain()
        test.test_emailformat_langchain_agent()
    ...
    try:
        test = TestEmailformatCrewAI()
        test.test_emailformat_crewai_agent()
