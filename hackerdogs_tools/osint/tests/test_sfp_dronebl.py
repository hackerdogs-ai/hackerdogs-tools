"""
Test suite for DroneBL tool (dronebl)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dronebl_langchain import sfp_dronebl
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dronebl_crewai import SfpDroneblTool
...
class TestDroneblStandalone:
    """Test DroneBL tool standalone execution."""
    
    def test_dronebl_standalone(self):
        """Test dronebl tool execution without agent."""
        ...
        result = sfp_dronebl.invoke(invoke_params)
        ...
        result_file = save_test_result("dronebl", "standalone", result_data, target_filename)
...
class TestDroneblLangChain:
    """Test DroneBL tool with LangChain agent."""
    
    def test_dronebl_langchain_agent(self):
        """Test dronebl tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DroneBL tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DroneBL to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dronebl", "langchain", result_data, target_filename)
...
class TestDroneblCrewAI:
    """Test DroneBL tool with CrewAI agent."""
    
    def test_dronebl_crewai_agent(self):
        """Test dronebl tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DroneBL",
        ...
        task = Task(
            description=f"Use DroneBL to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DroneBL tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dronebl", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DroneBL Tool Tests (dronebl)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDroneblStandalone()
        test.test_dronebl_standalone()
    ...
    try:
        test = TestDroneblLangChain()
        test.test_dronebl_langchain_agent()
    ...
    try:
        test = TestDroneblCrewAI()
        test.test_dronebl_crewai_agent()
