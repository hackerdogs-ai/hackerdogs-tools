"""
Test suite for DNS Zone Transfer tool (dnszonexfer)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnszonexfer_langchain import sfp_dnszonexfer
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnszonexfer_crewai import SfpDnszonexferTool
...
class TestDnszonexferStandalone:
    """Test DNS Zone Transfer tool standalone execution."""
    
    def test_dnszonexfer_standalone(self):
        """Test dnszonexfer tool execution without agent."""
        ...
        # Timeout in seconds
        invoke_params["timeout"] = 30
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_dnszonexfer.invoke(invoke_params)
        ...
        result_file = save_test_result("dnszonexfer", "standalone", result_data, target_filename)
...
class TestDnszonexferLangChain:
    """Test DNS Zone Transfer tool with LangChain agent."""
    
    def test_dnszonexfer_langchain_agent(self):
        """Test dnszonexfer tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_dnszonexfer]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNS Zone Transfer tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNS Zone Transfer to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnszonexfer", "langchain", result_data, target_filename)
...
class TestDnszonexferCrewAI:
    """Test DNS Zone Transfer tool with CrewAI agent."""
    
    def test_dnszonexfer_crewai_agent(self):
        """Test dnszonexfer tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNS Zone Transfer",
        ...
        task = Task(
            description=f"Use DNS Zone Transfer to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNS Zone Transfer tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnszonexfer", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNS Zone Transfer Tool Tests (dnszonexfer)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnszonexferStandalone()
        test.test_dnszonexfer_standalone()
    ...
    try:
        test = TestDnszonexferLangChain()
        test.test_dnszonexfer_langchain_agent()
    ...
    try:
        test = TestDnszonexferCrewAI()
        test.test_dnszonexfer_crewai_agent()
