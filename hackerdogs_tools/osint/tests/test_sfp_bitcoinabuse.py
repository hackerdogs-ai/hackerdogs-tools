"""
Test suite for BitcoinAbuse tool (bitcoinabuse)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bitcoinabuse_langchain import sfp_bitcoinabuse
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bitcoinabuse_crewai import SfpBitcoinabuseTool
...
class TestBitcoinabuseStandalone:
    """Test BitcoinAbuse tool standalone execution."""
    
    def test_bitcoinabuse_standalone(self):
        """Test bitcoinabuse tool execution without agent."""
        ...
        result = sfp_bitcoinabuse.invoke(invoke_params)
        ...
        result_file = save_test_result("bitcoinabuse", "standalone", result_data, target_filename)
...
class TestBitcoinabuseLangChain:
    """Test BitcoinAbuse tool with LangChain agent."""
    
    def test_bitcoinabuse_langchain_agent(self):
        """Test bitcoinabuse tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_bitcoinabuse]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the BitcoinAbuse tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use BitcoinAbuse to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("bitcoinabuse", "langchain", result_data, target_filename)
...
class TestBitcoinabuseCrewAI:
    """Test BitcoinAbuse tool with CrewAI agent."""
    
    def test_bitcoinabuse_crewai_agent(self):
        """Test bitcoinabuse tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using BitcoinAbuse",
        ...
        task = Task(
            description=f"Use BitcoinAbuse to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from BitcoinAbuse tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("bitcoinabuse", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running BitcoinAbuse Tool Tests (bitcoinabuse)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBitcoinabuseStandalone()
        test.test_bitcoinabuse_standalone()
    ...
    try:
        test = TestBitcoinabuseLangChain()
        test.test_bitcoinabuse_langchain_agent()
    ...
    try:
        test = TestBitcoinabuseCrewAI()
        test.test_bitcoinabuse_crewai_agent()
