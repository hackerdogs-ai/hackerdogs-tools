"""
Test suite for Base64 Decoder tool (base64)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_base64_langchain import sfp_base64
from hackerdogs_tools.osint.spiderfoot_modules.sfp_base64_crewai import SfpBase64Tool
...
class TestBase64Standalone:
    """Test Base64 Decoder tool standalone execution."""
    
    def test_base64_standalone(self):
        """Test base64 tool execution without agent."""
        ...
        # The minimum length a string that looks like a base64-encoded string needs to be.
        invoke_params["minlength"] = 10
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_base64.invoke(invoke_params)
        ...
        result_file = save_test_result("base64", "standalone", result_data, target_filename)
...
class TestBase64LangChain:
    """Test Base64 Decoder tool with LangChain agent."""
    
    def test_base64_langchain_agent(self):
        """Test base64 tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Base64 Decoder tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Base64 Decoder to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("base64", "langchain", result_data, target_filename)
...
class TestBase64CrewAI:
    """Test Base64 Decoder tool with CrewAI agent."""
    
    def test_base64_crewai_agent(self):
        """Test base64 tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Base64 Decoder",
        ...
        task = Task(
            description=f"Use Base64 Decoder to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Base64 Decoder tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("base64", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Base64 Decoder Tool Tests (base64)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBase64Standalone()
        test.test_base64_standalone()
    ...
    try:
        test = TestBase64LangChain()
        test.test_base64_langchain_agent()
    ...
    try:
        test = TestBase64CrewAI()
        test.test_base64_crewai_agent()
