"""
Test suite for Bitcoin Who's Who tool (bitcoinwhoswho)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bitcoinwhoswho_langchain import sfp_bitcoinwhoswho
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bitcoinwhoswho_crewai import SfpBitcoinwhoswhoTool
...
class TestBitcoinwhoswhoStandalone:
    """Test Bitcoin Who's Who tool standalone execution."""
    
    def test_bitcoinwhoswho_standalone(self):
        """Test bitcoinwhoswho tool execution without agent."""
        ...
        result = sfp_bitcoinwhoswho.invoke(invoke_params)
        ...
        result_file = save_test_result("bitcoinwhoswho", "standalone", result_data, target_filename)
...
class TestBitcoinwhoswhoLangChain:
    """Test Bitcoin Who's Who tool with LangChain agent."""
    
    def test_bitcoinwhoswho_langchain_agent(self):
        """Test bitcoinwhoswho tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_bitcoinwhoswho]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Bitcoin Who's Who tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Bitcoin Who's Who to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("bitcoinwhoswho", "langchain", result_data, target_filename)
...
class TestBitcoinwhoswhoCrewAI:
    """Test Bitcoin Who's Who tool with CrewAI agent."""
    
    def test_bitcoinwhoswho_crewai_agent(self):
        """Test bitcoinwhoswho tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Bitcoin Who's Who",
        ...
        task = Task(
            description=f"Use Bitcoin Who's Who to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Bitcoin Who's Who tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("bitcoinwhoswho", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Bitcoin Who's Who Tool Tests (bitcoinwhoswho)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBitcoinwhoswhoStandalone()
        test.test_bitcoinwhoswho_standalone()
    ...
    try:
        test = TestBitcoinwhoswhoLangChain()
        test.test_bitcoinwhoswho_langchain_agent()
    ...
    try:
        test = TestBitcoinwhoswhoCrewAI()
        test.test_bitcoinwhoswho_crewai_agent()
