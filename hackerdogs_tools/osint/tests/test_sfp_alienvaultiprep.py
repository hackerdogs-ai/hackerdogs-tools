"""
Test suite for AlienVault IP Reputation tool (alienvaultiprep)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_alienvaultiprep_langchain import sfp_alienvaultiprep
from hackerdogs_tools.osint.spiderfoot_modules.sfp_alienvaultiprep_crewai import SfpAlienvaultiprepTool
...
class TestAlienvaultiprepStandalone:
    """Test AlienVault IP Reputation tool standalone execution."""
    
    def test_alienvaultiprep_standalone(self):
        """Test alienvaultiprep tool execution without agent."""
        ...
        invoke_params["cacheperiod"] = 18
        # Report if any malicious IPs are found within owned netblocks?
        invoke_params["checknetblocks"] = True
        # Check if any malicious IPs are found within the same subnet of the target?
        invoke_params["checksubnets"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_alienvaultiprep.invoke(invoke_params)
        ...
        result_file = save_test_result("alienvaultiprep", "standalone", result_data, target_filename)
...
class TestAlienvaultiprepLangChain:
    """Test AlienVault IP Reputation tool with LangChain agent."""
    
    def test_alienvaultiprep_langchain_agent(self):
        """Test alienvaultiprep tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_alienvaultiprep]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the AlienVault IP Reputation tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use AlienVault IP Reputation to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("alienvaultiprep", "langchain", result_data, target_filename)
...
class TestAlienvaultiprepCrewAI:
    """Test AlienVault IP Reputation tool with CrewAI agent."""
    
    def test_alienvaultiprep_crewai_agent(self):
        """Test alienvaultiprep tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using AlienVault IP Reputation",
        ...
        task = Task(
            description=f"Use AlienVault IP Reputation to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from AlienVault IP Reputation tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("alienvaultiprep", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running AlienVault IP Reputation Tool Tests (alienvaultiprep)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAlienvaultiprepStandalone()
        test.test_alienvaultiprep_standalone()
    ...
    try:
        test = TestAlienvaultiprepLangChain()
        test.test_alienvaultiprep_langchain_agent()
    ...
    try:
        test = TestAlienvaultiprepCrewAI()
        test.test_alienvaultiprep_crewai_agent()
