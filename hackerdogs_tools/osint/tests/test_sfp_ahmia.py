"""
Test suite for Ahmia tool (ahmia)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_ahmia_langchain import sfp_ahmia
from hackerdogs_tools.osint.spiderfoot_modules.sfp_ahmia_crewai import SfpAhmiaTool
...
class TestAhmiaStandalone:
    """Test Ahmia tool standalone execution."""
    
    def test_ahmia_standalone(self):
        """Test ahmia tool execution without agent."""
        ...
        # Fetch the darknet pages (via TOR, if enabled) to verify they mention your target.
        invoke_params["fetchlinks"] = True
        # Search for human names?
        invoke_params["fullnames"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_ahmia.invoke(invoke_params)
        ...
        result_file = save_test_result("ahmia", "standalone", result_data, target_filename)
...
class TestAhmiaLangChain:
    """Test Ahmia tool with LangChain agent."""
    
    def test_ahmia_langchain_agent(self):
        """Test ahmia tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Ahmia tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Ahmia to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("ahmia", "langchain", result_data, target_filename)
...
class TestAhmiaCrewAI:
    """Test Ahmia tool with CrewAI agent."""
    
    def test_ahmia_crewai_agent(self):
        """Test ahmia tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Ahmia",
        ...
        task = Task(
            description=f"Use Ahmia to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Ahmia tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("ahmia", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Ahmia Tool Tests (ahmia)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAhmiaStandalone()
        test.test_ahmia_standalone()
    ...
    try:
        test = TestAhmiaLangChain()
        test.test_ahmia_langchain_agent()
    ...
    try:
        test = TestAhmiaCrewAI()
        test.test_ahmia_crewai_agent()
