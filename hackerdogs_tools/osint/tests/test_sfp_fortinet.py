"""
Test suite for FortiGuard Antispam tool (fortinet)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_fortinet_langchain import sfp_fortinet
from hackerdogs_tools.osint.spiderfoot_modules.sfp_fortinet_crewai import SfpFortinetTool
...
class TestFortinetStandalone:
    """Test FortiGuard Antispam tool standalone execution."""
    
    def test_fortinet_standalone(self):
        """Test fortinet tool execution without agent."""
        ...
        result = sfp_fortinet.invoke(invoke_params)
        ...
        result_file = save_test_result("fortinet", "standalone", result_data, target_filename)
...
class TestFortinetLangChain:
    """Test FortiGuard Antispam tool with LangChain agent."""
    
    def test_fortinet_langchain_agent(self):
        """Test fortinet tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the FortiGuard Antispam tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use FortiGuard Antispam to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("fortinet", "langchain", result_data, target_filename)
...
class TestFortinetCrewAI:
    """Test FortiGuard Antispam tool with CrewAI agent."""
    
    def test_fortinet_crewai_agent(self):
        """Test fortinet tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using FortiGuard Antispam",
        ...
        task = Task(
            description=f"Use FortiGuard Antispam to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from FortiGuard Antispam tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("fortinet", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running FortiGuard Antispam Tool Tests (fortinet)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestFortinetStandalone()
        test.test_fortinet_standalone()
    ...
    try:
        test = TestFortinetLangChain()
        test.test_fortinet_langchain_agent()
    ...
    try:
        test = TestFortinetCrewAI()
        test.test_fortinet_crewai_agent()
