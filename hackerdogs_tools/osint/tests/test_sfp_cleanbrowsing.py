"""
Test suite for CleanBrowsing.org tool (cleanbrowsing)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cleanbrowsing_langchain import sfp_cleanbrowsing
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cleanbrowsing_crewai import SfpCleanbrowsingTool
...
class TestCleanbrowsingStandalone:
    """Test CleanBrowsing.org tool standalone execution."""
    
    def test_cleanbrowsing_standalone(self):
        """Test cleanbrowsing tool execution without agent."""
        ...
        result = sfp_cleanbrowsing.invoke(invoke_params)
        ...
        result_file = save_test_result("cleanbrowsing", "standalone", result_data, target_filename)
...
class TestCleanbrowsingLangChain:
    """Test CleanBrowsing.org tool with LangChain agent."""
    
    def test_cleanbrowsing_langchain_agent(self):
        """Test cleanbrowsing tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_cleanbrowsing]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CleanBrowsing.org tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CleanBrowsing.org to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("cleanbrowsing", "langchain", result_data, target_filename)
...
class TestCleanbrowsingCrewAI:
    """Test CleanBrowsing.org tool with CrewAI agent."""
    
    def test_cleanbrowsing_crewai_agent(self):
        """Test cleanbrowsing tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CleanBrowsing.org",
        ...
        task = Task(
            description=f"Use CleanBrowsing.org to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CleanBrowsing.org tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("cleanbrowsing", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CleanBrowsing.org Tool Tests (cleanbrowsing)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCleanbrowsingStandalone()
        test.test_cleanbrowsing_standalone()
    ...
    try:
        test = TestCleanbrowsingLangChain()
        test.test_cleanbrowsing_langchain_agent()
    ...
    try:
        test = TestCleanbrowsingCrewAI()
        test.test_cleanbrowsing_crewai_agent()
