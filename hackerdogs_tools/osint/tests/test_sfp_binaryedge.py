"""
Test suite for BinaryEdge tool (binaryedge)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_binaryedge_langchain import sfp_binaryedge
from hackerdogs_tools.osint.spiderfoot_modules.sfp_binaryedge_crewai import SfpBinaryedgeTool
...
class TestBinaryedgeStandalone:
    """Test BinaryEdge tool standalone execution."""
    
    def test_binaryedge_standalone(self):
        """Test binaryedge tool execution without agent."""
        ...
        invoke_params["binaryedge_api_key"] = ""
        # Ignore any torrent records older than this many days. 0 = unlimited.
        invoke_params["torrent_age_limit_days"] = 30
        # Ignore any vulnerability records older than this many days. 0 = unlimited.
        invoke_params["cve_age_limit_days"] = 30
        # Ignore any discovered open ports/banners older than this many days. 0 = unlimited.
        invoke_params["port_age_limit_days"] = 90
        # Maximum number of pages to iterate through, to avoid exceeding BinaryEdge API usage limits. APIv2 has a maximum of 500 pages (10,000 results).
        invoke_params["maxpages"] = 10
        # Verify that any hostnames found on the target domain still resolve?
        invoke_params["verify"] = True
        ...
        invoke_params["netblocklookup"] = False
        # If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)
        invoke_params["maxnetblock"] = 24
        # Look up all IPs on subnets which your target is a part of?
        invoke_params["subnetlookup"] = False
        # If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)
        invoke_params["maxsubnet"] = 24
        # Stop reporting co-hosted sites after this many are found, as it would likely indicate web hosting.
        invoke_params["maxcohost"] = 100
        # API key will be retrieved from runtime.state or environment
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_binaryedge.invoke(invoke_params)
        ...
        result_file = save_test_result("binaryedge", "standalone", result_data, target_filename)
...
class TestBinaryedgeLangChain:
    """Test BinaryEdge tool with LangChain agent."""
    
    def test_binaryedge_langchain_agent(self):
        """Test binaryedge tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_binaryedge]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the BinaryEdge tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use BinaryEdge to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("binaryedge", "langchain", result_data, target_filename)
...
class TestBinaryedgeCrewAI:
    """Test BinaryEdge tool with CrewAI agent."""
    
    def test_binaryedge_crewai_agent(self):
        """Test binaryedge tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using BinaryEdge",
        ...
        task = Task(
            description=f"Use BinaryEdge to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from BinaryEdge tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("binaryedge", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running BinaryEdge Tool Tests (binaryedge)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBinaryedgeStandalone()
        test.test_binaryedge_standalone()
    ...
    try:
        test = TestBinaryedgeLangChain()
        test.test_binaryedge_langchain_agent()
    ...
    try:
        test = TestBinaryedgeCrewAI()
        test.test_binaryedge_crewai_agent()
