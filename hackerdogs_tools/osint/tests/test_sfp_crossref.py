"""
Test suite for Cross-Referencer tool (crossref)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crossref_langchain import sfp_crossref
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crossref_crewai import SfpCrossrefTool
...
class TestCrossrefStandalone:
    """Test Cross-Referencer tool standalone execution."""
    
    def test_crossref_standalone(self):
        """Test crossref tool execution without agent."""
        ...
        # Check the base URL of the potential affiliate if no direct affiliation found?
        invoke_params["checkbase"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_crossref.invoke(invoke_params)
        ...
        result_file = save_test_result("crossref", "standalone", result_data, target_filename)
...
class TestCrossrefLangChain:
    """Test Cross-Referencer tool with LangChain agent."""
    
    def test_crossref_langchain_agent(self):
        """Test crossref tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Cross-Referencer tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Cross-Referencer to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("crossref", "langchain", result_data, target_filename)
...
class TestCrossrefCrewAI:
    """Test Cross-Referencer tool with CrewAI agent."""
    
    def test_crossref_crewai_agent(self):
        """Test crossref tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Cross-Referencer",
        ...
        task = Task(
            description=f"Use Cross-Referencer to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Cross-Referencer tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("crossref", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Cross-Referencer Tool Tests (crossref)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCrossrefStandalone()
        test.test_crossref_standalone()
    ...
    try:
        test = TestCrossrefLangChain()
        test.test_crossref_langchain_agent()
    ...
    try:
        test = TestCrossrefCrewAI()
        test.test_crossref_crewai_agent()
