"""
Test suite for Digital Ocean Space Finder tool (digitaloceanspace)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_digitaloceanspace_langchain import sfp_digitaloceanspace
from hackerdogs_tools.osint.spiderfoot_modules.sfp_digitaloceanspace_crewai import SfpDigitaloceanspaceTool
...
class TestDigitaloceanspaceStandalone:
    """Test Digital Ocean Space Finder tool standalone execution."""
    
    def test_digitaloceanspace_standalone(self):
        """Test digitaloceanspace tool execution without agent."""
        ...
        # Different Digital Ocean locations to check where spaces may exist.
        invoke_params["endpoints"] = "nyc3.digitaloceanspaces.com,sgp1.digitaloceanspaces.com,ams3.digitaloceanspaces.com"
        # List of suffixes to append to domains tried as space names
        invoke_params["suffixes"] = "test,dev,web,beta,bucket,space,files,content,data,prod,staging,production,stage,app,media,development,-test,-dev,-web,-beta,-bucket,-space,-files,-content,-data,-prod,-staging,-production,-stage,-app,-media,-development"
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_digitaloceanspace.invoke(invoke_params)
        ...
        result_file = save_test_result("digitaloceanspace", "standalone", result_data, target_filename)
...
class TestDigitaloceanspaceLangChain:
    """Test Digital Ocean Space Finder tool with LangChain agent."""
    
    def test_digitaloceanspace_langchain_agent(self):
        """Test digitaloceanspace tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_digitaloceanspace]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Digital Ocean Space Finder tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Digital Ocean Space Finder to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("digitaloceanspace", "langchain", result_data, target_filename)
...
class TestDigitaloceanspaceCrewAI:
    """Test Digital Ocean Space Finder tool with CrewAI agent."""
    
    def test_digitaloceanspace_crewai_agent(self):
        """Test digitaloceanspace tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Digital Ocean Space Finder",
            ...
            tools=[SfpDigitaloceanspaceTool()],
            llm=llm,
            verbose=True
        )
        ...
        task = Task(
            description=f"Use Digital Ocean Space Finder to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Digital Ocean Space Finder tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("digitaloceanspace", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Digital Ocean Space Finder Tool Tests (digitaloceanspace)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDigitaloceanspaceStandalone()
        test.test_digitaloceanspace_standalone()
    ...
    try:
        test = TestDigitaloceanspaceLangChain()
        test.test_digitaloceanspace_langchain_agent()
    ...
    try:
        test = TestDigitaloceanspaceCrewAI()
        test.test_digitaloceanspace_crewai_agent()
