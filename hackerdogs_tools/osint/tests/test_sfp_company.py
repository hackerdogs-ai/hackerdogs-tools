"""
Test suite for Company Name Extractor tool (company)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_company_langchain import sfp_company
from hackerdogs_tools.osint.spiderfoot_modules.sfp_company_crewai import SfpCompanyTool
...
class TestCompanyStandalone:
    """Test Company Name Extractor tool standalone execution."""
    
    def test_company_standalone(self):
        """Test company tool execution without agent."""
        ...
        # Filter out company names that originated from CSS/JS content. Enabling this avoids detection of popular Javascript and web framework author company names.
        invoke_params["filterjscss"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_company.invoke(invoke_params)
        ...
        result_file = save_test_result("company", "standalone", result_data, target_filename)
...
class TestCompanyLangChain:
    """Test Company Name Extractor tool with LangChain agent."""
    
    def test_company_langchain_agent(self):
        """Test company tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Company Name Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Company Name Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("company", "langchain", result_data, target_filename)
...
class TestCompanyCrewAI:
    """Test Company Name Extractor tool with CrewAI agent."""
    
    def test_company_crewai_agent(self):
        """Test company tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Company Name Extractor",
        ...
        task = Task(
            description=f"Use Company Name Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Company Name Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("company", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Company Name Extractor Tool Tests (company)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCompanyStandalone()
        test.test_company_standalone()
    ...
    try:
        test = TestCompanyLangChain()
        test.test_company_langchain_agent()
    ...
    try:
        test = TestCompanyCrewAI()
        test.test_company_crewai_agent()
