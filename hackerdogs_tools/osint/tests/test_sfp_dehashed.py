"""
Test suite for Dehashed tool (dehashed)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dehashed_langchain import sfp_dehashed
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dehashed_crewai import SfpDehashedTool
...
class TestDehashedStandalone:
    """Test Dehashed tool standalone execution."""
    
    def test_dehashed_standalone(self):
        """Test dehashed tool execution without agent."""
        ...
        result_file = save_test_result("dehashed", "standalone", result_data, target_filename)
...
class TestDehashedLangChain:
    """Test Dehashed tool with LangChain agent."""
    
    def test_dehashed_langchain_agent(self):
        """Test dehashed tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Dehashed tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Dehashed to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dehashed", "langchain", result_data, target_filename)
...
class TestDehashedCrewAI:
    """Test Dehashed tool with CrewAI agent."""
    
    def test_dehashed_crewai_agent(self):
        """Test dehashed tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Dehashed",
        ...
        task = Task(
            description=f"Use Dehashed to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Dehashed tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dehashed", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Dehashed Tool Tests (dehashed)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDehashedStandalone()
        test.test_dehashed_standalone()
    ...
    try:
        test = TestDehashedLangChain()
        test.test_dehashed_langchain_agent()
    ...
    try:
        test = TestDehashedCrewAI()
        test.test_dehashed_crewai_agent()
