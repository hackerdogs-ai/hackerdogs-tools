"""
Test suite for Account Finder tool (accounts)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_accounts_langchain import sfp_accounts
from hackerdogs_tools.osint.spiderfoot_modules.sfp_accounts_crewai import SfpAccountsTool
...
class TestAccountsStandalone:
    """Test Account Finder tool standalone execution."""
    
    def test_accounts_standalone(self):
        """Test accounts tool execution without agent."""
        ...
        # Don't bother looking up names that are just stand-alone first names (too many false positives).
        invoke_params["ignorenamedict"] = True
        # Don't bother looking up names that appear in the dictionary.
        invoke_params["ignoreworddict"] = True
        # The username must be mentioned on the social media page to consider it valid (helps avoid false positives).
        invoke_params["musthavename"] = True
        # Extract usernames from e-mail addresses at all? If disabled this can reduce false positives for common usernames but for highly unique usernames it would result in missed accounts.
        invoke_params["userfromemail"] = True
        # Look for the existence of account name permutations. Useful to identify fraudulent social media accounts or account squatting.
        invoke_params["permutate"] = False
        # The minimum length of a username to query across social media sites. Helps avoid false positives for very common short usernames.
        invoke_params["usernamesize"] = 4
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_accounts.invoke(invoke_params)
        ...
        result_file = save_test_result("accounts", "standalone", result_data, target_filename)
...
class TestAccountsLangChain:
    """Test Account Finder tool with LangChain agent."""
    
    def test_accounts_langchain_agent(self):
        """Test accounts tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Account Finder tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Account Finder to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("accounts", "langchain", result_data, target_filename)
...
class TestAccountsCrewAI:
    """Test Account Finder tool with CrewAI agent."""
    
    def test_accounts_crewai_agent(self):
        """Test accounts tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Account Finder",
        ...
        task = Task(
            description=f"Use Account Finder to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Account Finder tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("accounts", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Account Finder Tool Tests (accounts)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAccountsStandalone()
        test.test_accounts_standalone()
    ...
    try:
        test = TestAccountsLangChain()
        test.test_accounts_langchain_agent()
    ...
    try:
        test = TestAccountsCrewAI()
        test.test_accounts_crewai_agent()
