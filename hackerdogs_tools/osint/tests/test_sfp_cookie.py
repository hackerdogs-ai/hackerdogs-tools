"""
Test suite for Cookie Extractor tool (cookie)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cookie_langchain import sfp_cookie
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cookie_crewai import SfpCookieTool
...
class TestCookieStandalone:
    """Test Cookie Extractor tool standalone execution."""
    
    def test_cookie_standalone(self):
        """Test cookie tool execution without agent."""
        ...
        result = sfp_cookie.invoke(invoke_params)
        ...
        result_file = save_test_result("cookie", "standalone", result_data, target_filename)
...
class TestCookieLangChain:
    """Test Cookie Extractor tool with LangChain agent."""
    
    def test_cookie_langchain_agent(self):
        """Test cookie tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Cookie Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Cookie Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("cookie", "langchain", result_data, target_filename)
...
class TestCookieCrewAI:
    """Test Cookie Extractor tool with CrewAI agent."""
    
    def test_cookie_crewai_agent(self):
        """Test cookie tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Cookie Extractor",
        ...
        task = Task(
            description=f"Use Cookie Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Cookie Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("cookie", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Cookie Extractor Tool Tests (cookie)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCookieStandalone()
        test.test_cookie_standalone()
    ...
    try:
        test = TestCookieLangChain()
        test.test_cookie_langchain_agent()
    ...
    try:
        test = TestCookieCrewAI()
        test.test_cookie_crewai_agent()
