"""
Test suite for DNSDB tool (dnsdb)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsdb_langchain import sfp_dnsdb
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsdb_crewai import SfpDnsdbTool
...
class TestDnsdbStandalone:
    """Test DNSDB tool standalone execution."""
    
    def test_dnsdb_standalone(self):
        """Test dnsdb tool execution without agent."""
        ...
        # Ignore any DNSDB records older than this many days. 0 = unlimited.
        ...
        result = sfp_dnsdb.invoke(invoke_params)
        ...
        result_file = save_test_result("dnsdb", "standalone", result_data, target_filename)
...
class TestDnsdbLangChain:
    """Test DNSDB tool with LangChain agent."""
    
    def test_dnsdb_langchain_agent(self):
        """Test dnsdb tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNSDB tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNSDB to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnsdb", "langchain", result_data, target_filename)
...
class TestDnsdbCrewAI:
    """Test DNSDB tool with CrewAI agent."""
    
    def test_dnsdb_crewai_agent(self):
        """Test dnsdb tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNSDB",
        ...
        task = Task(
            description=f"Use DNSDB to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNSDB tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnsdb", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNSDB Tool Tests (dnsdb)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnsdbStandalone()
        test.test_dnsdb_standalone()
    ...
    try:
        test = TestDnsdbLangChain()
        test.test_dnsdb_langchain_agent()
    ...
    try:
        test = TestDnsdbCrewAI()
        test.test_dnsdb_crewai_agent()
