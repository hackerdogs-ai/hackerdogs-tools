"""
Test suite for ThreatCrowd tool (threatcrowd)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_threatcrowd_langchain import sfp_threatcrowd
from hackerdogs_tools.osint.spiderfoot_modules.sfp_threatcrowd_crewai import SfpThreatcrowdTool
...
class TestThreatcrowdStandalone:
    """Test ThreatCrowd tool standalone execution."""
    
    def test_threatcrowd_standalone(self):
        """Test threatcrowd tool execution without agent."""
        ...
        result = sfp_threatcrowd.invoke(invoke_params)
        ...
        result_file = save_test_result("threatcrowd", "standalone", result_data, target_filename)
...
class TestThreatcrowdLangChain:
    """Test ThreatCrowd tool with LangChain agent."""
    
    def test_threatcrowd_langchain_agent(self):
        """Test threatcrowd tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_threatcrowd]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the ThreatCrowd tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use ThreatCrowd to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("threatcrowd", "langchain", result_data, target_filename)
...
class TestThreatcrowdCrewAI:
    """Test ThreatCrowd tool with CrewAI agent."""
    
    def test_threatcrowd_crewai_agent(self):
        """Test threatcrowd tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using ThreatCrowd",
        ...
        task = Task(
            description=f"Use ThreatCrowd to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from ThreatCrowd tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("threatcrowd", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running ThreatCrowd Tool Tests (threatcrowd)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestThreatcrowdStandalone()
        test.test_threatcrowd_standalone()
    ...
    try:
        test = TestThreatcrowdLangChain()
        test.test_threatcrowd_langchain_agent()
    ...
    try:
        test = TestThreatcrowdCrewAI()
        test.test_threatcrowd_crewai_agent()
