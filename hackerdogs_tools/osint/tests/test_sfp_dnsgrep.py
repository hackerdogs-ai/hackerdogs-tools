"""
Test suite for DNSGrep tool (dnsgrep)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsgrep_langchain import sfp_dnsgrep
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsgrep_crewai import SfpDnsgrepTool
...
class TestDnsgrepStandalone:
    """Test DNSGrep tool standalone execution."""
    
    def test_dnsgrep_standalone(self):
        """Test dnsgrep tool execution without agent."""
        ...
        result = sfp_dnsgrep.invoke(invoke_params)
        ...
        result_file = save_test_result("dnsgrep", "standalone", result_data, target_filename)
...
class TestDnsgrepLangChain:
    """Test DNSGrep tool with LangChain agent."""
    
    def test_dnsgrep_langchain_agent(self):
        """Test dnsgrep tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNSGrep tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNSGrep to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnsgrep", "langchain", result_data, target_filename)
...
class TestDnsgrepCrewAI:
    """Test DNSGrep tool with CrewAI agent."""
    
    def test_dnsgrep_crewai_agent(self):
        """Test dnsgrep tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNSGrep",
        ...
        task = Task(
            description=f"Use DNSGrep to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNSGrep tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnsgrep", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNSGrep Tool Tests (dnsgrep)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnsgrepStandalone()
        test.test_dnsgrep_standalone()
    ...
    try:
        test = TestDnsgrepLangChain()
        test.test_dnsgrep_langchain_agent()
    ...
    try:
        test = TestDnsgrepCrewAI()
        test.test_dnsgrep_crewai_agent()
