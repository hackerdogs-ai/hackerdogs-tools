"""
Test suite for Apple iTunes tool (apple_itunes)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_apple_itunes_langchain import sfp_apple_itunes
from hackerdogs_tools.osint.spiderfoot_modules.sfp_apple_itunes_crewai import SfpAppleitunesTool
...
class TestAppleitunesStandalone:
    """Test Apple iTunes tool standalone execution."""
    
    def test_apple_itunes_standalone(self):
        """Test apple_itunes tool execution without agent."""
        ...
        result = sfp_apple_itunes.invoke(invoke_params)
        ...
        result_file = save_test_result("apple_itunes", "standalone", result_data, target_filename)
...
class TestAppleitunesLangChain:
    """Test Apple iTunes tool with LangChain agent."""
    
    def test_apple_itunes_langchain_agent(self):
        """Test apple_itunes tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_apple_itunes]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Apple iTunes tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Apple iTunes to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("apple_itunes", "langchain", result_data, target_filename)
...
class TestAppleitunesCrewAI:
    """Test Apple iTunes tool with CrewAI agent."""
    
    def test_apple_itunes_crewai_agent(self):
        """Test apple_itunes tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Apple iTunes",
        ...
        task = Task(
            description=f"Use Apple iTunes to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Apple iTunes tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("apple_itunes", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Apple iTunes Tool Tests (apple_itunes)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAppleitunesStandalone()
        test.test_apple_itunes_standalone()
    ...
    try:
        test = TestAppleitunesLangChain()
        test.test_apple_itunes_langchain_agent()
    ...
    try:
        test = TestAppleitunesCrewAI()
        test.test_apple_itunes_crewai_agent()
