"""
Test suite for DNS Brute-forcer tool (dnsbrute)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsbrute_langchain import sfp_dnsbrute
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsbrute_crewai import SfpDnsbruteTool
...
class TestDnsbruteStandalone:
    """Test DNS Brute-forcer tool standalone execution."""
    
    def test_dnsbrute_standalone(self):
        """Test dnsbrute tool execution without agent."""
        ...
        # If wildcard DNS is detected, don't bother brute-forcing.
        invoke_params["skipcommonwildcard"] = True
        # Only attempt to brute-force names on domain names, not hostnames (some hostnames are also sub-domains).
        invoke_params["domainonly"] = True
        # Try a list of about 750 common hostnames/sub-domains.
        invoke_params["commons"] = True
        # Try a further 10,000 common hostnames/sub-domains. Will make the scan much slower.
        invoke_params["top10000"] = False
        # For any host found, try appending 1, 01, 001, -1, -01, -001, 2, 02, etc. (up to 10)
        invoke_params["numbersuffix"] = True
        # Limit using the number suffixes for hosts that have already been resolved? If disabled this will significantly extend the duration of scans.
        invoke_params["numbersuffixlimit"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_dnsbrute.invoke(invoke_params)
        ...
        result_file = save_test_result("dnsbrute", "standalone", result_data, target_filename)
...
class TestDnsbruteLangChain:
    """Test DNS Brute-forcer tool with LangChain agent."""
    
    def test_dnsbrute_langchain_agent(self):
        """Test dnsbrute tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNS Brute-forcer tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNS Brute-forcer to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnsbrute", "langchain", result_data, target_filename)
...
class TestDnsbruteCrewAI:
    """Test DNS Brute-forcer tool with CrewAI agent."""
    
    def test_dnsbrute_crewai_agent(self):
        """Test dnsbrute tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNS Brute-forcer",
        ...
        task = Task(
            description=f"Use DNS Brute-forcer to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNS Brute-forcer tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnsbrute", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNS Brute-forcer Tool Tests (dnsbrute)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnsbruteStandalone()
        test.test_dnsbrute_standalone()
    ...
    try:
        test = TestDnsbruteLangChain()
        test.test_dnsbrute_langchain_agent()
    ...
    try:
        test = TestDnsbruteCrewAI()
        test.test_dnsbrute_crewai_agent()
