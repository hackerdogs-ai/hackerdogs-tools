"""
Test suite for AbstractAPI tool (abstractapi)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_abstractapi_langchain import sfp_abstractapi
from hackerdogs_tools.osint.spiderfoot_modules.sfp_abstractapi_crewai import SfpAbstractapiTool
...
class TestAbstractapiStandalone:
    """Test AbstractAPI tool standalone execution."""
    
    def test_abstractapi_standalone(self):
        """Test abstractapi tool execution without agent."""
        ...
        # AbstractAPI Company Enrichment API key.
        invoke_params["companyenrichment_api_key"] = ""
        # AbstractAPI Phone Validation API key.
        invoke_params["phonevalidation_api_key"] = ""
        # AbstractAPI IP Geolocation API key.
        invoke_params["ipgeolocation_api_key"] = ""
        # API key will be retrieved from runtime.state or environment
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_abstractapi.invoke(invoke_params)
        ...
        result_file = save_test_result("abstractapi", "standalone", result_data, target_filename)
...
class TestAbstractapiLangChain:
    """Test AbstractAPI tool with LangChain agent."""
    
    def test_abstractapi_langchain_agent(self):
        """Test abstractapi tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_abstractapi]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the AbstractAPI tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use AbstractAPI to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("abstractapi", "langchain", result_data, target_filename)
...
class TestAbstractapiCrewAI:
    """Test AbstractAPI tool with CrewAI agent."""
    
    def test_abstractapi_crewai_agent(self):
        """Test abstractapi tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using AbstractAPI",
        ...
        task = Task(
            description=f"Use AbstractAPI to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from AbstractAPI tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("abstractapi", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running AbstractAPI Tool Tests (abstractapi)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAbstractapiStandalone()
        test.test_abstractapi_standalone()
    ...
    try:
        test = TestAbstractapiLangChain()
        test.test_abstractapi_langchain_agent()
    ...
    try:
        test = TestAbstractapiCrewAI()
        test.test_abstractapi_crewai_agent()
