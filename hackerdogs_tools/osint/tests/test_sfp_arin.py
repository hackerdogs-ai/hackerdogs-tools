...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_arin_langchain import sfp_arin
from hackerdogs_tools.osint.spiderfoot_modules.sfp_arin_crewai import SfpArinTool
...
class TestArinStandalone:
    """Test ARIN tool standalone execution."""
    
    def test_arin_standalone(self):
        """Test arin tool execution without agent."""
        ...
        result = sfp_arin.invoke(invoke_params)
        ...
        result_file = save_test_result("arin", "standalone", result_data, target_filename)
...
class TestArinLangChain:
    """Test ARIN tool with LangChain agent."""
    
    def test_arin_langchain_agent(self):
        """Test arin tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the ARIN tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use ARIN to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("arin", "langchain", result_data, target_filename)
...
class TestArinCrewAI:
    """Test ARIN tool with CrewAI agent."""
    
    def test_arin_crewai_agent(self):
        """Test arin tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using ARIN",
        ...
        task = Task(
            description=f"Use ARIN to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from ARIN tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("arin", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running ARIN Tool Tests (arin)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestArinStandalone()
        test.test_arin_standalone()
    ...
    try:
        test = TestArinLangChain()
        test.test_arin_langchain_agent()
