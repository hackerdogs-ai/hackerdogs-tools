"""
Test suite for AlienVault OTX tool (alienvault)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_alienvault_langchain import sfp_alienvault
from hackerdogs_tools.osint.spiderfoot_modules.sfp_alienvault_crewai import SfpAlienvaultTool
...
class TestAlienvaultStandalone:
    """Test AlienVault OTX tool standalone execution."""
    
    def test_alienvault_standalone(self):
        """Test alienvault tool execution without agent."""
        ...
        # Verify co-hosts are valid by checking if they still resolve to the shared IP.
        invoke_params["verify"] = True
        # Ignore any reputation records older than this many days. 0 = unlimited.
        invoke_params["reputation_age_limit_days"] = 30
        # Ignore any co-hosts older than this many days. 0 = unlimited.
        invoke_params["cohost_age_limit_days"] = 30
        # Minimum AlienVault threat score.
        invoke_params["threat_score_min"] = 2
        ...
        # Maximum number of pages of URL results to fetch.
        invoke_params["max_pages"] = 50
        ...
        result = sfp_alienvault.invoke(invoke_params)
        ...
        result_file = save_test_result("alienvault", "standalone", result_data, target_filename)
...
class TestAlienvaultLangChain:
    """Test AlienVault OTX tool with LangChain agent."""
    
    def test_alienvault_langchain_agent(self):
        """Test alienvault tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_alienvault]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the AlienVault OTX tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use AlienVault OTX to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("alienvault", "langchain", result_data, target_filename)
...
class TestAlienvaultCrewAI:
    """Test AlienVault OTX tool with CrewAI agent."""
    
    def test_alienvault_crewai_agent(self):
        """Test alienvault tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using AlienVault OTX",
        ...
        task = Task(
            description=f"Use AlienVault OTX to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from AlienVault OTX tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("alienvault", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running AlienVault OTX Tool Tests (alienvault)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAlienvaultStandalone()
        test.test_alienvault_standalone()
    ...
    try:
        test = TestAlienvaultLangChain()
        test.test_alienvault_langchain_agent()
    ...
    try:
        test = TestAlienvaultCrewAI()
        test.test_alienvault_crewai_agent()
