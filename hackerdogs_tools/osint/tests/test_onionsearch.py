...
class TestOnionsearchStandalone:
    ...
    def test_onionsearch_standalone(self):
        ...
        result = onionsearch_search(
            runtime=runtime,
            query="test", engines=None, max_results=50
        )
...
class TestOnionsearchLangChain:
    """Test onionsearch tool with LangChain agent."""
    ...
    @pytest.fixture
    def agent(self, llm):
        """Create LangChain agent with onionsearch tool."""
    ...
    def test_onionsearch_langchain_agent(self, agent):
        """Test onionsearch tool with LangChain agent."""
        runtime = ToolRuntime(state={"user_id": "test_user"})
        ...
        result = executor.invoke({
            "messages": [HumanMessage(content="Search dark web for test using OnionSearch")],
            "runtime": runtime
        })
...
class TestOnionsearchCrewAI:
    ...
    def test_onionsearch_crewai_agent(self, agent, llm):
        """Test onionsearch tool with CrewAI agent."""
        task = Task(
            description="Search dark web for test using OnionSearch",
            agent=agent,
...
def run_all_tests():
    ...
    try:
        ...
        test = TestOnionsearchLangChain()
        test.test_onionsearch_langchain_agent(test.agent(llm))
    ...
    try:
        ...
        test = TestOnionsearchCrewAI()
        test.test_onionsearch_crewai_agent(test.agent(llm), llm)
