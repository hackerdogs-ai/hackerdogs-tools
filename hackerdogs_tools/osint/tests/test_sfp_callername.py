"""
Test suite for CallerName tool (callername)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_callername_langchain import sfp_callername
from hackerdogs_tools.osint.spiderfoot_modules.sfp_callername_crewai import SfpCallernameTool
...
class TestCallernameStandalone:
    """Test CallerName tool standalone execution."""
    
    def test_callername_standalone(self):
        """Test callername tool execution without agent."""
        ...
        result = sfp_callername.invoke(invoke_params)
        ...
        result_file = save_test_result("callername", "standalone", result_data, target_filename)
...
class TestCallernameLangChain:
    """Test CallerName tool with LangChain agent."""
    
    def test_callername_langchain_agent(self):
        """Test callername tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_callername]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CallerName tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CallerName to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("callername", "langchain", result_data, target_filename)
...
class TestCallernameCrewAI:
    """Test CallerName tool with CrewAI agent."""
    
    def test_callername_crewai_agent(self):
        """Test callername tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CallerName",
        ...
        task = Task(
            description=f"Use CallerName to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CallerName tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("callername", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CallerName Tool Tests (callername)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCallernameStandalone()
        test.test_callername_standalone()
    ...
    try:
        test = TestCallernameLangChain()
        test.test_callername_langchain_agent()
    ...
    try:
        test = TestCallernameCrewAI()
        test.test_callername_crewai_agent()
