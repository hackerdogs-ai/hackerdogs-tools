"""
Test suite for Clearbit tool (clearbit)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_clearbit_langchain import sfp_clearbit
from hackerdogs_tools.osint.spiderfoot_modules.sfp_clearbit_crewai import SfpClearbitTool
...
class TestClearbitStandalone:
    """Test Clearbit tool standalone execution."""
    
    def test_clearbit_standalone(self):
        """Test clearbit tool execution without agent."""
        ...
        result = sfp_clearbit.invoke(invoke_params)
        ...
        result_file = save_test_result("clearbit", "standalone", result_data, target_filename)
...
class TestClearbitLangChain:
    """Test Clearbit tool with LangChain agent."""
    
    def test_clearbit_langchain_agent(self):
        """Test clearbit tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Clearbit tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Clearbit to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("clearbit", "langchain", result_data, target_filename)
...
class TestClearbitCrewAI:
    """Test Clearbit tool with CrewAI agent."""
    
    def test_clearbit_crewai_agent(self):
        """Test clearbit tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Clearbit",
        ...
        task = Task(
            description=f"Use Clearbit to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Clearbit tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("clearbit", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Clearbit Tool Tests (clearbit)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestClearbitStandalone()
        test.test_clearbit_standalone()
    ...
    try:
        test = TestClearbitLangChain()
        test.test_clearbit_langchain_agent()
    ...
    try:
        test = TestClearbitCrewAI()
        test.test_clearbit_crewai_agent()
