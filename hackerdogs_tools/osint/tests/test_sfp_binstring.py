"""
Test suite for Binary String Extractor tool (binstring)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_binstring_langchain import sfp_binstring
from hackerdogs_tools.osint.spiderfoot_modules.sfp_binstring_crewai import SfpBinstringTool
...
class TestBinstringStandalone:
    """Test Binary String Extractor tool standalone execution."""
    
    def test_binstring_standalone(self):
        """Test binstring tool execution without agent."""
        ...
        # Upon finding a string in a binary, ensure it is at least this length. Helps weed out false positives.
        invoke_params["minwordsize"] = 5
        # Stop reporting strings from a single binary after this many are found.
        invoke_params["maxwords"] = 100
        # Maximum file size in bytes to download for analysis.
        invoke_params["maxfilesize"] = 1000000
        # Use the dictionary to further reduce false positives - any string found must contain a word from the dictionary (can be very slow, especially for larger files).
        invoke_params["usedict"] = True
        # File types to fetch and analyse.
        invoke_params["fileexts"] = ['png', 'gif', 'jpg', 'jpeg', 'tiff', 'tif', 'ico', 'flv', 'mp4', 'mp3', 'avi', 'mpg', 'mpeg', 'dat', 'mov', 'swf', 'exe', 'bin']
        # Ignore strings with these characters, as they may just be garbage ASCII.
        invoke_params["filterchars"] = "#}{|%^&*()=+,;[]~"
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_binstring.invoke(invoke_params)
        ...
        result_file = save_test_result("binstring", "standalone", result_data, target_filename)
...
class TestBinstringLangChain:
    """Test Binary String Extractor tool with LangChain agent."""
    
    def test_binstring_langchain_agent(self):
        """Test binstring tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Binary String Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Binary String Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("binstring", "langchain", result_data, target_filename)
...
class TestBinstringCrewAI:
    """Test Binary String Extractor tool with CrewAI agent."""
    
    def test_binstring_crewai_agent(self):
        """Test binstring tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Binary String Extractor",
        ...
        task = Task(
            description=f"Use Binary String Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Binary String Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("binstring", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Binary String Extractor Tool Tests (binstring)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBinstringStandalone()
        test.test_binstring_standalone()
    ...
    try:
        test = TestBinstringLangChain()
        test.test_binstring_langchain_agent()
    ...
    try:
        test = TestBinstringCrewAI()
        test.test_binstring_crewai_agent()
