"""
Test suite for Bing (Shared IPs) tool (bingsharedip)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bingsharedip_langchain import sfp_bingsharedip
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bingsharedip_crewai import SfpBingsharedipTool
...
class TestBingsharedipStandalone:
    """Test Bing (Shared IPs) tool standalone execution."""
    
    def test_bingsharedip_standalone(self):
        """Test bingsharedip tool execution without agent."""
        ...
        invoke_params["cohostsamedomain"] = False
        # Number of max bing results to request from API.
        invoke_params["pages"] = 20
        ...
        result = sfp_bingsharedip.invoke(invoke_params)
        ...
        result_file = save_test_result("bingsharedip", "standalone", result_data, target_filename)
...
class TestBingsharedipLangChain:
    """Test Bing (Shared IPs) tool with LangChain agent."""
    
    def test_bingsharedip_langchain_agent(self):
        """Test bingsharedip tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_bingsharedip]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Bing (Shared IPs) tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Bing (Shared IPs) to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("bingsharedip", "langchain", result_data, target_filename)
...
class TestBingsharedipCrewAI:
    """Test Bing (Shared IPs) tool with CrewAI agent."""
    
    def test_bingsharedip_crewai_agent(self):
        """Test bingsharedip tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Bing (Shared IPs)",
        ...
        task = Task(
            description=f"Use Bing (Shared IPs) to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Bing (Shared IPs) tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("bingsharedip", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Bing (Shared IPs) Tool Tests (bingsharedip)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBingsharedipStandalone()
        test.test_bingsharedip_standalone()
    ...
    try:
        test = TestBingsharedipLangChain()
        test.test_bingsharedip_langchain_agent()
    ...
    try:
        test = TestBingsharedipCrewAI()
        test.test_bingsharedip_crewai_agent()
