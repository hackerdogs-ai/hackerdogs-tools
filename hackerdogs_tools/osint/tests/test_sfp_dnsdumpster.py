"""
Test suite for DNSDumpster tool (dnsdumpster)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsdumpster_langchain import sfp_dnsdumpster
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsdumpster_crewai import SfpDnsdumpsterTool
...
class TestDnsdumpsterStandalone:
    ...
    def test_dnsdumpster_standalone(self):
        ...
        result = sfp_dnsdumpster.invoke(invoke_params)
        ...
        result_file = save_test_result("dnsdumpster", "standalone", result_data, target_filename)
...
class TestDnsdumpsterLangChain:
    ...
    def test_dnsdumpster_langchain_agent(self):
        """Test dnsdumpster tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_dnsdumpster]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNSDumpster tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNSDumpster to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnsdumpster", "langchain", result_data, target_filename)
...
class TestDnsdumpsterCrewAI:
    ...
    def test_dnsdumpster_crewai_agent(self):
        """Test dnsdumpster tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        ...
        task = Task(
            description=f"Use DNSDumpster to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNSDumpster tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnsdumpster", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNSDumpster Tool Tests (dnsdumpster)")
    print("=" * 80)
    ...
    try:
        ...
        test.test_dnsdumpster_langchain_agent()
    ...
    try:
        ...
        test.test_dnsdumpster_crewai_agent()
