"""
Test suite for DNS Resolver tool (dnsresolve)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsresolve_langchain import sfp_dnsresolve
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsresolve_crewai import SfpDnsresolveTool
...
class TestDnsresolveStandalone:
    """Test DNS Resolver tool standalone execution."""
    
    def test_dnsresolve_standalone(self):
        """Test dnsresolve tool execution without agent."""
        ...
        # Validate that reverse-resolved hostnames still resolve back to that IP before considering them as aliases of your target.
        invoke_params["validatereverse"] = True
        # If wildcard DNS is detected, only attempt to look up the first common sub-domain from the common sub-domain list.
        invoke_params["skipcommononwildcard"] = True
        ...
        # Maximum owned IPv4 netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)
        invoke_params["maxnetblock"] = 24
        # Maximum owned IPv6 netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)
        invoke_params["maxv6netblock"] = 120
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_dnsresolve.invoke(invoke_params)
        ...
        result_file = save_test_result("dnsresolve", "standalone", result_data, target_filename)
...
class TestDnsresolveLangChain:
    """Test DNS Resolver tool with LangChain agent."""
    
    def test_dnsresolve_langchain_agent(self):
        """Test dnsresolve tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_dnsresolve]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNS Resolver tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNS Resolver to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnsresolve", "langchain", result_data, target_filename)
...
class TestDnsresolveCrewAI:
    """Test DNS Resolver tool with CrewAI agent."""
    
    def test_dnsresolve_crewai_agent(self):
        """Test dnsresolve tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNS Resolver",
        ...
        task = Task(
            description=f"Use DNS Resolver to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNS Resolver tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnsresolve", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNS Resolver Tool Tests (dnsresolve)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnsresolveStandalone()
        test.test_dnsresolve_standalone()
    ...
    try:
        test = TestDnsresolveLangChain()
        test.test_dnsresolve_langchain_agent()
    ...
    try:
        test = TestDnsresolveCrewAI()
        test.test_dnsresolve_crewai_agent()
