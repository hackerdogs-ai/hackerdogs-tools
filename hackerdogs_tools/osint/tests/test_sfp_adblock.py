"""
Test suite for AdBlock Check tool (adblock)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_adblock_langchain import sfp_adblock
from hackerdogs_tools.osint.spiderfoot_modules.sfp_adblock_crewai import SfpAdblockTool
...
class TestAdblockStandalone:
    """Test AdBlock Check tool standalone execution."""
    
    def test_adblock_standalone(self):
        """Test adblock tool execution without agent."""
        ...
        # AdBlockPlus block list.
        invoke_params["blocklist"] = "https://easylist-downloads.adblockplus.org/easylist.txt"
        # Hours to cache list data before re-fetching.
        invoke_params["cacheperiod"] = 24
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_adblock.invoke(invoke_params)
        ...
        result_file = save_test_result("adblock", "standalone", result_data, target_filename)
...
class TestAdblockLangChain:
    """Test AdBlock Check tool with LangChain agent."""
    
    def test_adblock_langchain_agent(self):
        """Test adblock tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the AdBlock Check tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use AdBlock Check to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("adblock", "langchain", result_data, target_filename)
...
class TestAdblockCrewAI:
    """Test AdBlock Check tool with CrewAI agent."""
    
    def test_adblock_crewai_agent(self):
        """Test adblock tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using AdBlock Check",
        ...
        task = Task(
            description=f"Use AdBlock Check to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from AdBlock Check tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("adblock", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running AdBlock Check Tool Tests (adblock)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAdblockStandalone()
        test.test_adblock_standalone()
    ...
    try:
        test = TestAdblockLangChain()
        test.test_adblock_langchain_agent()
    ...
    try:
        test = TestAdblockCrewAI()
        test.test_adblock_crewai_agent()
