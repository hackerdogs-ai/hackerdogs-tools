"""
Test suite for CINS Army List tool (cinsscore)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cinsscore_langchain import sfp_cinsscore
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cinsscore_crewai import SfpCinsscoreTool
...
class TestCinsscoreStandalone:
    """Test CINS Army List tool standalone execution."""
    
    def test_cinsscore_standalone(self):
        """Test cinsscore tool execution without agent."""
        ...
        result = sfp_cinsscore.invoke(invoke_params)
        ...
        result_file = save_test_result("cinsscore", "standalone", result_data, target_filename)
...
class TestCinsscoreLangChain:
    """Test CINS Army List tool with LangChain agent."""
    
    def test_cinsscore_langchain_agent(self):
        """Test cinsscore tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CINS Army List tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CINS Army List to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("cinsscore", "langchain", result_data, target_filename)
...
class TestCinsscoreCrewAI:
    """Test CINS Army List tool with CrewAI agent."""
    
    def test_cinsscore_crewai_agent(self):
        """Test cinsscore tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CINS Army List",
        ...
        task = Task(
            description=f"Use CINS Army List to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CINS Army List tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("cinsscore", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CINS Army List Tool Tests (cinsscore)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCinsscoreStandalone()
        test.test_cinsscore_standalone()
    ...
    try:
        test = TestCinsscoreLangChain()
        test.test_cinsscore_langchain_agent()
    ...
    try:
        test = TestCinsscoreCrewAI()
        test.test_cinsscore_crewai_agent()
