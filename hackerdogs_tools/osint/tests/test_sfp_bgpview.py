"""
Test suite for BGPView tool (bgpview)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bgpview_langchain import sfp_bgpview
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bgpview_crewai import SfpBgpviewTool
...
class TestBgpviewStandalone:
    """Test BGPView tool standalone execution."""
    
    def test_bgpview_standalone(self):
        """Test bgpview tool execution without agent."""
        ...
        result = sfp_bgpview.invoke(invoke_params)
        ...
        result_file = save_test_result("bgpview", "standalone", result_data, target_filename)
...
class TestBgpviewLangChain:
    """Test BGPView tool with LangChain agent."""
    
    def test_bgpview_langchain_agent(self):
        """Test bgpview tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the BGPView tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use BGPView to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("bgpview", "langchain", result_data, target_filename)
...
class TestBgpviewCrewAI:
    """Test BGPView tool with CrewAI agent."""
    
    def test_bgpview_crewai_agent(self):
        """Test bgpview tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using BGPView",
        ...
        task = Task(
            description=f"Use BGPView to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from BGPView tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("bgpview", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running BGPView Tool Tests (bgpview)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBgpviewStandalone()
        test.test_bgpview_standalone()
    ...
    try:
        test = TestBgpviewLangChain()
        test.test_bgpview_langchain_agent()
    ...
    try:
        test = TestBgpviewCrewAI()
        test.test_bgpview_crewai_agent()
