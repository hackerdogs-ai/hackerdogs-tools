"""
Test suite for Azure Blob Finder tool (azureblobstorage)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_azureblobstorage_langchain import sfp_azureblobstorage
from hackerdogs_tools.osint.spiderfoot_modules.sfp_azureblobstorage_crewai import SfpAzureblobstorageTool
...
class TestAzureblobstorageStandalone:
    """Test Azure Blob Finder tool standalone execution."""
    
    def test_azureblobstorage_standalone(self):
        """Test azureblobstorage tool execution without agent."""
        ...
        # List of suffixes to append to domains tried as blob storage names
        ...
        result = sfp_azureblobstorage.invoke(invoke_params)
        ...
        result_file = save_test_result("azureblobstorage", "standalone", result_data, target_filename)
...
class TestAzureblobstorageLangChain:
    """Test Azure Blob Finder tool with LangChain agent."""
    
    def test_azureblobstorage_langchain_agent(self):
        """Test azureblobstorage tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_azureblobstorage]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Azure Blob Finder tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Azure Blob Finder to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("azureblobstorage", "langchain", result_data, target_filename)
...
class TestAzureblobstorageCrewAI:
    """Test Azure Blob Finder tool with CrewAI agent."""
    
    def test_azureblobstorage_crewai_agent(self):
        """Test azureblobstorage tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Azure Blob Finder",
        ...
        task = Task(
            description=f"Use Azure Blob Finder to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Azure Blob Finder tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("azureblobstorage", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Azure Blob Finder Tool Tests (azureblobstorage)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAzureblobstorageStandalone()
        test.test_azureblobstorage_standalone()
    ...
    try:
        test = TestAzureblobstorageLangChain()
        test.test_azureblobstorage_langchain_agent()
    ...
    try:
        test = TestAzureblobstorageCrewAI()
        test.test_azureblobstorage_crewai_agent()
