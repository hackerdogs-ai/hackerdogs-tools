"""
Test suite for SHODAN tool (shodan)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_shodan_langchain import sfp_shodan
from hackerdogs_tools.osint.spiderfoot_modules.sfp_shodan_crewai import SfpShodanTool
...
class TestShodanStandalone:
    """Test SHODAN tool standalone execution."""
    
    def test_shodan_standalone(self):
        """Test shodan tool execution without agent."""
        ...
        result = sfp_shodan.invoke(invoke_params)
        ...
        result_file = save_test_result("shodan", "standalone", result_data, target_filename)
...
class TestShodanLangChain:
    """Test SHODAN tool with LangChain agent."""
    
    def test_shodan_langchain_agent(self):
        """Test shodan tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the SHODAN tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use SHODAN to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("shodan", "langchain", result_data, target_filename)
...
class TestShodanCrewAI:
    """Test SHODAN tool with CrewAI agent."""
    
    def test_shodan_crewai_agent(self):
        """Test shodan tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using SHODAN",
        ...
        task = Task(
            description=f"Use SHODAN to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from SHODAN tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("shodan", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running SHODAN Tool Tests (shodan)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestShodanStandalone()
        test.test_shodan_standalone()
    ...
    try:
        test = TestShodanLangChain()
        test.test_shodan_langchain_agent()
    ...
    try:
        test = TestShodanCrewAI()
        test.test_shodan_crewai_agent()
