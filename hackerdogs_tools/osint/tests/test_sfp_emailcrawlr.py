"""
Test suite for EmailCrawlr tool (emailcrawlr)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emailcrawlr_langchain import sfp_emailcrawlr
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emailcrawlr_crewai import SfpEmailcrawlrTool
...
class TestEmailcrawlrStandalone:
    """Test EmailCrawlr tool standalone execution."""
    
    def test_emailcrawlr_standalone(self):
        """Test emailcrawlr tool execution without agent."""
        ...
        result = sfp_emailcrawlr.invoke(invoke_params)
        ...
        result_file = save_test_result("emailcrawlr", "standalone", result_data, target_filename)
...
class TestEmailcrawlrLangChain:
    """Test EmailCrawlr tool with LangChain agent."""
    
    def test_emailcrawlr_langchain_agent(self):
        """Test emailcrawlr tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_emailcrawlr]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the EmailCrawlr tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use EmailCrawlr to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("emailcrawlr", "langchain", result_data, target_filename)
...
class TestEmailcrawlrCrewAI:
    """Test EmailCrawlr tool with CrewAI agent."""
    
    def test_emailcrawlr_crewai_agent(self):
        """Test emailcrawlr tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using EmailCrawlr",
        ...
        task = Task(
            description=f"Use EmailCrawlr to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from EmailCrawlr tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("emailcrawlr", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running EmailCrawlr Tool Tests (emailcrawlr)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestEmailcrawlrStandalone()
        test.test_emailcrawlr_standalone()
    ...
    try:
        test = TestEmailcrawlrLangChain()
        test.test_emailcrawlr_langchain_agent()
    ...
    try:
        test = TestEmailcrawlrCrewAI()
        test.test_emailcrawlr_crewai_agent()
