"""
Test suite for Focsec tool (focsec)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_focsec_langchain import sfp_focsec
from hackerdogs_tools.osint.spiderfoot_modules.sfp_focsec_crewai import SfpFocsecTool
...
class TestFocsecStandalone:
    """Test Focsec tool standalone execution."""
    
    def test_focsec_standalone(self):
        """Test focsec tool execution without agent."""
        ...
        result = sfp_focsec.invoke(invoke_params)
        ...
        result_file = save_test_result("focsec", "standalone", result_data, target_filename)
...
class TestFocsecLangChain:
    """Test Focsec tool with LangChain agent."""
    
    def test_focsec_langchain_agent(self):
        """Test focsec tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Focsec tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Focsec to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("focsec", "langchain", result_data, target_filename)
...
class TestFocsecCrewAI:
    """Test Focsec tool with CrewAI agent."""
    
    def test_focsec_crewai_agent(self):
        """Test focsec tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Focsec",
        ...
        task = Task(
            description=f"Use Focsec to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Focsec tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("focsec", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Focsec Tool Tests (focsec)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestFocsecStandalone()
        test.test_focsec_standalone()
    ...
    try:
        test = TestFocsecLangChain()
        test.test_focsec_langchain_agent()
    ...
    try:
        test = TestFocsecCrewAI()
        test.test_focsec_crewai_agent()
