"""
Test suite for Archive.org tool (archiveorg)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_archiveorg_langchain import sfp_archiveorg
from hackerdogs_tools.osint.spiderfoot_modules.sfp_archiveorg_crewai import SfpArchiveorgTool
...
class TestArchiveorgStandalone:
    """Test Archive.org tool standalone execution."""
    
    def test_archiveorg_standalone(self):
        """Test archiveorg tool execution without agent."""
        ...
        # Number of days back to look for older versions of files/pages in the Wayback Machine snapshots. Comma-separate the values, so for example 30,60,90 means to look for snapshots 30 days, 60 days and 90 days back.
        invoke_params["farback"] = "30,60,90"
        # Query the Wayback Machine for historic versions of Interesting Files.
        invoke_params["intfiles"] = True
        # Query the Wayback Machine for historic versions of URLs with passwords.
        invoke_params["passwordpages"] = True
        # Query the Wayback Machine for historic versions of URLs with forms.
        invoke_params["formpages"] = False
        # Query the Wayback Machine for historic versions of URLs containing Flash.
        invoke_params["flashpages"] = False
        # Query the Wayback Machine for historic versions of URLs using Java Applets.
        invoke_params["javapages"] = False
        # Query the Wayback Machine for historic versions of purely static URLs.
        invoke_params["staticpages"] = False
        # Query the Wayback Machine for historic versions of URLs accepting uploads.
        invoke_params["uploadpages"] = False
        # Query the Wayback Machine for historic versions of URLs using Javascript frameworks.
        invoke_params["webframeworkpages"] = False
        # Query the Wayback Machine for historic versions of URLs using Javascript.
        invoke_params["javascriptpages"] = False
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_archiveorg.invoke(invoke_params)
        ...
        result_file = save_test_result("archiveorg", "standalone", result_data, target_filename)
...
class TestArchiveorgLangChain:
    """Test Archive.org tool with LangChain agent."""
    
    def test_archiveorg_langchain_agent(self):
        """Test archiveorg tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_archiveorg]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Archive.org tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Archive.org to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("archiveorg", "langchain", result_data, target_filename)
...
class TestArchiveorgCrewAI:
    """Test Archive.org tool with CrewAI agent."""
    
    def test_archiveorg_crewai_agent(self):
        """Test archiveorg tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Archive.org",
        ...
        task = Task(
            description=f"Use Archive.org to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Archive.org tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("archiveorg", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Archive.org Tool Tests (archiveorg)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestArchiveorgStandalone()
        test.test_archiveorg_standalone()
    ...
    try:
        test = TestArchiveorgLangChain()
        test.test_archiveorg_langchain_agent()
    ...
    try:
        test = TestArchiveorgCrewAI()
        test.test_archiveorg_crewai_agent()
