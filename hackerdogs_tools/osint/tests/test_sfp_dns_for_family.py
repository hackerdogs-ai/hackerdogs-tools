"""
Test suite for DNS for Family tool (dns_for_family)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dns_for_family_langchain import sfp_dns_for_family
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dns_for_family_crewai import SfpDnsforfamilyTool
...
class TestDnsforfamilyStandalone:
    """Test DNS for Family tool standalone execution."""
    
    def test_dns_for_family_standalone(self):
        """Test dns_for_family tool execution without agent."""
        ...
        result = sfp_dns_for_family.invoke(invoke_params)
        ...
        result_file = save_test_result("dns_for_family", "standalone", result_data, target_filename)
...
class TestDnsforfamilyLangChain:
    """Test DNS for Family tool with LangChain agent."""
    
    def test_dns_for_family_langchain_agent(self):
        """Test dns_for_family tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_dns_for_family]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNS for Family tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNS for Family to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dns_for_family", "langchain", result_data, target_filename)
...
class TestDnsforfamilyCrewAI:
    """Test DNS for Family tool with CrewAI agent."""
    
    def test_dns_for_family_crewai_agent(self):
        """Test dns_for_family tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNS for Family",
        ...
        task = Task(
            description=f"Use DNS for Family to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNS for Family tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dns_for_family", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNS for Family Tool Tests (dns_for_family)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnsforfamilyStandalone()
        test.test_dns_for_family_standalone()
    ...
    try:
        test = TestDnsforfamilyLangChain()
        test.test_dns_for_family_langchain_agent()
    ...
    try:
        test = TestDnsforfamilyCrewAI()
        test.test_dns_for_family_crewai_agent()
