"""
Test suite for botvrij.eu tool (botvrij)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_botvrij_langchain import sfp_botvrij
from hackerdogs_tools.osint.spiderfoot_modules.sfp_botvrij_crewai import SfpBotvrijTool
...
class TestBotvrijStandalone:
    """Test botvrij.eu tool standalone execution."""
    
    def test_botvrij_standalone(self):
        """Test botvrij tool execution without agent."""
        ...
        result = sfp_botvrij.invoke(invoke_params)
        ...
        result_file = save_test_result("botvrij", "standalone", result_data, target_filename)
...
class TestBotvrijLangChain:
    """Test botvrij.eu tool with LangChain agent."""
    
    def test_botvrij_langchain_agent(self):
        """Test botvrij tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the botvrij.eu tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use botvrij.eu to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("botvrij", "langchain", result_data, target_filename)
...
class TestBotvrijCrewAI:
    """Test botvrij.eu tool with CrewAI agent."""
    
    def test_botvrij_crewai_agent(self):
        """Test botvrij tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using botvrij.eu",
        ...
        task = Task(
            description=f"Use botvrij.eu to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from botvrij.eu tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("botvrij", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running botvrij.eu Tool Tests (botvrij)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBotvrijStandalone()
        test.test_botvrij_standalone()
    ...
    try:
        test = TestBotvrijLangChain()
        test.test_botvrij_langchain_agent()
    ...
    try:
        test = TestBotvrijCrewAI()
        test.test_botvrij_crewai_agent()
