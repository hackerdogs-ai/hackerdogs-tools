"""
Test suite for CloudFlare DNS tool (cloudflaredns)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cloudflaredns_langchain import sfp_cloudflaredns
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cloudflaredns_crewai import SfpCloudflarednsTool
...
class TestCloudflarednsStandalone:
    """Test CloudFlare DNS tool standalone execution."""
    
    def test_cloudflaredns_standalone(self):
        """Test cloudflaredns tool execution without agent."""
        ...
        result = sfp_cloudflaredns.invoke(invoke_params)
        ...
        result_file = save_test_result("cloudflaredns", "standalone", result_data, target_filename)
...
class TestCloudflarednsLangChain:
    """Test CloudFlare DNS tool with LangChain agent."""
    
    def test_cloudflaredns_langchain_agent(self):
        """Test cloudflaredns tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_cloudflaredns]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CloudFlare DNS tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CloudFlare DNS to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("cloudflaredns", "langchain", result_data, target_filename)
...
class TestCloudflarednsCrewAI:
    """Test CloudFlare DNS tool with CrewAI agent."""
    
    def test_cloudflaredns_crewai_agent(self):
        """Test cloudflaredns tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CloudFlare DNS",
        ...
        task = Task(
            description=f"Use CloudFlare DNS to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CloudFlare DNS tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("cloudflaredns", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CloudFlare DNS Tool Tests (cloudflaredns)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCloudflarednsStandalone()
        test.test_cloudflaredns_standalone()
    ...
    try:
        test = TestCloudflarednsLangChain()
        test.test_cloudflaredns_langchain_agent()
    ...
    try:
        test = TestCloudflarednsCrewAI()
        test.test_cloudflaredns_crewai_agent()
