"""
Test suite for blocklist.de tool (blocklistde)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_blocklistde_langchain import sfp_blocklistde
from hackerdogs_tools.osint.spiderfoot_modules.sfp_blocklistde_crewai import SfpBlocklistdeTool
...
class TestBlocklistdeStandalone:
    """Test blocklist.de tool standalone execution."""
    
    def test_blocklistde_standalone(self):
        """Test blocklistde tool execution without agent."""
        ...
        result = sfp_blocklistde.invoke(invoke_params)
        ...
        result_file = save_test_result("blocklistde", "standalone", result_data, target_filename)
...
class TestBlocklistdeLangChain:
    """Test blocklist.de tool with LangChain agent."""
    
    def test_blocklistde_langchain_agent(self):
        """Test blocklistde tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_blocklistde]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the blocklist.de tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use blocklist.de to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("blocklistde", "langchain", result_data, target_filename)
...
class TestBlocklistdeCrewAI:
    """Test blocklist.de tool with CrewAI agent."""
    
    def test_blocklistde_crewai_agent(self):
        """Test blocklistde tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using blocklist.de",
        ...
        task = Task(
            description=f"Use blocklist.de to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from blocklist.de tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("blocklistde", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running blocklist.de Tool Tests (blocklistde)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBlocklistdeStandalone()
        test.test_blocklistde_standalone()
    ...
    try:
        test = TestBlocklistdeLangChain()
        test.test_blocklistde_langchain_agent()
    ...
    try:
        test = TestBlocklistdeCrewAI()
        test.test_blocklistde_crewai_agent()
