"""
Test suite for CleanTalk Spam List tool (cleantalk)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cleantalk_langchain import sfp_cleantalk
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cleantalk_crewai import SfpCleantalkTool
...
class TestCleantalkStandalone:
    """Test CleanTalk Spam List tool standalone execution."""
    
    def test_cleantalk_standalone(self):
        """Test cleantalk tool execution without agent."""
        ...
        result = sfp_cleantalk.invoke(invoke_params)
        ...
        result_file = save_test_result("cleantalk", "standalone", result_data, target_filename)
...
class TestCleantalkLangChain:
    """Test CleanTalk Spam List tool with LangChain agent."""
    
    def test_cleantalk_langchain_agent(self):
        """Test cleantalk tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CleanTalk Spam List tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CleanTalk Spam List to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("cleantalk", "langchain", result_data, target_filename)
...
class TestCleantalkCrewAI:
    """Test CleanTalk Spam List tool with CrewAI agent."""
    
    def test_cleantalk_crewai_agent(self):
        """Test cleantalk tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CleanTalk Spam List",
        ...
        task = Task(
            description=f"Use CleanTalk Spam List to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CleanTalk Spam List tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("cleantalk", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CleanTalk Spam List Tool Tests (cleantalk)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCleantalkStandalone()
        test.test_cleantalk_standalone()
    ...
    try:
        test = TestCleantalkLangChain()
        test.test_cleantalk_langchain_agent()
    ...
    try:
        test = TestCleantalkCrewAI()
        test.test_cleantalk_crewai_agent()
