"""
Test suite for Bitcoin Finder tool (bitcoin)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bitcoin_langchain import sfp_bitcoin
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bitcoin_crewai import SfpBitcoinTool
...
class TestBitcoinStandalone:
    """Test Bitcoin Finder tool standalone execution."""
    
    def test_bitcoin_standalone(self):
        """Test bitcoin tool execution without agent."""
        ...
        result = sfp_bitcoin.invoke(invoke_params)
        ...
        result_file = save_test_result("bitcoin", "standalone", result_data, target_filename)
...
class TestBitcoinLangChain:
    """Test Bitcoin Finder tool with LangChain agent."""
    
    def test_bitcoin_langchain_agent(self):
        """Test bitcoin tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Bitcoin Finder tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Bitcoin Finder to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("bitcoin", "langchain", result_data, target_filename)
...
class TestBitcoinCrewAI:
    """Test Bitcoin Finder tool with CrewAI agent."""
    
    def test_bitcoin_crewai_agent(self):
        """Test bitcoin tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Bitcoin Finder",
        ...
        task = Task(
            description=f"Use Bitcoin Finder to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Bitcoin Finder tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("bitcoin", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Bitcoin Finder Tool Tests (bitcoin)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBitcoinStandalone()
        test.test_bitcoin_standalone()
    ...
    try:
        test = TestBitcoinLangChain()
        test.test_bitcoin_langchain_agent()
    ...
    try:
        test = TestBitcoinCrewAI()
        test.test_bitcoin_crewai_agent()
