"""
Test suite for Leak-Lookup tool (citadel)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_citadel_langchain import sfp_citadel
from hackerdogs_tools.osint.spiderfoot_modules.sfp_citadel_crewai import SfpCitadelTool
...
class TestCitadelStandalone:
    """Test Leak-Lookup tool standalone execution."""
    
    def test_citadel_standalone(self):
        """Test citadel tool execution without agent."""
        ...
        result_file = save_test_result("citadel", "standalone", result_data, target_filename)
...
class TestCitadelLangChain:
    """Test Leak-Lookup tool with LangChain agent."""
    
    def test_citadel_langchain_agent(self):
        """Test citadel tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Leak-Lookup tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Leak-Lookup to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("citadel", "langchain", result_data, target_filename)
...
class TestCitadelCrewAI:
    """Test Leak-Lookup tool with CrewAI agent."""
    
    def test_citadel_crewai_agent(self):
        """Test citadel tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Leak-Lookup",
        ...
        task = Task(
            description=f"Use Leak-Lookup to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Leak-Lookup tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("citadel", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Leak-Lookup Tool Tests (citadel)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCitadelStandalone()
        test.test_citadel_standalone()
    ...
    try:
        test = TestCitadelLangChain()
        test.test_citadel_langchain_agent()
    ...
    try:
        test = TestCitadelCrewAI()
        test.test_citadel_crewai_agent()
