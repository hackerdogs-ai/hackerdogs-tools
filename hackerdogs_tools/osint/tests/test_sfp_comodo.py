"""
Test suite for Comodo Secure DNS tool (comodo)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_comodo_langchain import sfp_comodo
from hackerdogs_tools.osint.spiderfoot_modules.sfp_comodo_crewai import SfpComodoTool
...
class TestComodoStandalone:
    """Test Comodo Secure DNS tool standalone execution."""
    
    def test_comodo_standalone(self):
        """Test comodo tool execution without agent."""
        ...
        result = sfp_comodo.invoke(invoke_params)
        ...
        result_file = save_test_result("comodo", "standalone", result_data, target_filename)
...
class TestComodoLangChain:
    """Test Comodo Secure DNS tool with LangChain agent."""
    
    def test_comodo_langchain_agent(self):
        """Test comodo tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Comodo Secure DNS tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Comodo Secure DNS to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("comodo", "langchain", result_data, target_filename)
...
class TestComodoCrewAI:
    """Test Comodo Secure DNS tool with CrewAI agent."""
    
    def test_comodo_crewai_agent(self):
        """Test comodo tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Comodo Secure DNS",
        ...
        task = Task(
            description=f"Use Comodo Secure DNS to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Comodo Secure DNS tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("comodo", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Comodo Secure DNS Tool Tests (comodo)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestComodoStandalone()
        test.test_comodo_standalone()
    ...
    try:
        test = TestComodoLangChain()
        test.test_comodo_langchain_agent()
    ...
    try:
        test = TestComodoCrewAI()
        test.test_comodo_crewai_agent()
