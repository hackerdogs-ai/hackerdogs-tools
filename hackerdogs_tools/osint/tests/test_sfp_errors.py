"""
Test suite for Error String Extractor tool (errors)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_errors_langchain import sfp_errors
from hackerdogs_tools.osint.spiderfoot_modules.sfp_errors_crewai import SfpErrorsTool
...
class TestErrorsStandalone:
    """Test Error String Extractor tool standalone execution."""
    
    def test_errors_standalone(self):
        """Test errors tool execution without agent."""
        ...
        result = sfp_errors.invoke(invoke_params)
        ...
        result_file = save_test_result("errors", "standalone", result_data, target_filename)
...
class TestErrorsLangChain:
    """Test Error String Extractor tool with LangChain agent."""
    
    def test_errors_langchain_agent(self):
        """Test errors tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Error String Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Error String Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("errors", "langchain", result_data, target_filename)
...
class TestErrorsCrewAI:
    """Test Error String Extractor tool with CrewAI agent."""
    
    def test_errors_crewai_agent(self):
        """Test errors tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Error String Extractor",
        ...
        task = Task(
            description=f"Use Error String Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Error String Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("errors", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Error String Extractor Tool Tests (errors)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestErrorsStandalone()
        test.test_errors_standalone()
    ...
    try:
        test = TestErrorsLangChain()
        test.test_errors_langchain_agent()
    ...
    try:
        test = TestErrorsCrewAI()
        test.test_errors_crewai_agent()
