"""
Test suite for CIRCL.LU tool (circllu)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_circllu_langchain import sfp_circllu
from hackerdogs_tools.osint.spiderfoot_modules.sfp_circllu_crewai import SfpCirclluTool
...
class TestCirclluStandalone:
    """Test CIRCL.LU tool standalone execution."""
    
    def test_circllu_standalone(self):
        """Test circllu tool execution without agent."""
        ...
        # CIRCL.LU login.
        invoke_params["api_key_login"] = ""
        # CIRCL.LU password.
        invoke_params["api_key_password"] = ""
        # Ignore any Passive DNS records older than this many days. 0 = unlimited.
        invoke_params["age_limit_days"] = 0
        ...
        result = sfp_circllu.invoke(invoke_params)
        ...
        result_file = save_test_result("circllu", "standalone", result_data, target_filename)
...
class TestCirclluLangChain:
    """Test CIRCL.LU tool with LangChain agent."""
    
    def test_circllu_langchain_agent(self):
        """Test circllu tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CIRCL.LU tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CIRCL.LU to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("circllu", "langchain", result_data, target_filename)
...
class TestCirclluCrewAI:
    """Test CIRCL.LU tool with CrewAI agent."""
    
    def test_circllu_crewai_agent(self):
        """Test circllu tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CIRCL.LU",
        ...
        task = Task(
            description=f"Use CIRCL.LU to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CIRCL.LU tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("circllu", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CIRCL.LU Tool Tests (circllu)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCirclluStandalone()
        test.test_circllu_standalone()
    ...
    try:
        test = TestCirclluLangChain()
        test.test_circllu_langchain_agent()
    ...
    try:
        test = TestCirclluCrewAI()
        test.test_circllu_crewai_agent()
