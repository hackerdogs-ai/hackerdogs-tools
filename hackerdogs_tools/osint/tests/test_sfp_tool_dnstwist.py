"""
Test suite for Tool - DNSTwist tool (tool_dnstwist)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_tool_dnstwist_langchain import sfp_tool_dnstwist
from hackerdogs_tools.osint.spiderfoot_modules.sfp_tool_dnstwist_crewai import SfpTooldnstwistTool
...
class TestTooldnstwistStandalone:
    """Test Tool - DNSTwist tool standalone execution."""
    
    def test_tool_dnstwist_standalone(self):
        """Test tool_dnstwist tool execution without agent."""
        ...
        # Path to Python interpreter to use for DNSTwist. If just 'python' then it must be in your PATH.
        invoke_params["pythonpath"] = "python"
        # Path to the where the dnstwist.py file lives. Optional.
        invoke_params["dnstwistpath"] = ""
        # Skip TLDs and sub-TLDs that have wildcard DNS.
        invoke_params["skipwildcards"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_tool_dnstwist.invoke(invoke_params)
        ...
        result_file = save_test_result("tool_dnstwist", "standalone", result_data, target_filename)
...
class TestTooldnstwistLangChain:
    """Test Tool - DNSTwist tool with LangChain agent."""
    
    def test_tool_dnstwist_langchain_agent(self):
        """Test tool_dnstwist tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_tool_dnstwist]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Tool - DNSTwist tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Tool - DNSTwist to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("tool_dnstwist", "langchain", result_data, target_filename)
...
class TestTooldnstwistCrewAI:
    """Test Tool - DNSTwist tool with CrewAI agent."""
    
    def test_tool_dnstwist_crewai_agent(self):
        """Test tool_dnstwist tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Tool - DNSTwist",
        ...
        task = Task(
            description=f"Use Tool - DNSTwist to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Tool - DNSTwist tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("tool_dnstwist", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Tool - DNSTwist Tool Tests (tool_dnstwist)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestTooldnstwistStandalone()
        test.test_tool_dnstwist_standalone()
    ...
    try:
        test = TestTooldnstwistLangChain()
        test.test_tool_dnstwist_langchain_agent()
    ...
    try:
        test = TestTooldnstwistCrewAI()
        test.test_tool_dnstwist_crewai_agent()
