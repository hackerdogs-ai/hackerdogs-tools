"""
Test suite for Censys tool (censys)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_censys_langchain import sfp_censys
from hackerdogs_tools.osint.spiderfoot_modules.sfp_censys_crewai import SfpCensysTool
...
class TestCensysStandalone:
    """Test Censys tool standalone execution."""
    
    def test_censys_standalone(self):
        """Test censys tool execution without agent."""
        ...
        # Censys.io API UID.
        invoke_params["censys_api_key_uid"] = ""
        # Censys.io API Secret.
        invoke_params["censys_api_key_secret"] = ""
        # Delay between requests, in seconds.
        invoke_params["delay"] = 3
        ...
        result = sfp_censys.invoke(invoke_params)
        ...
        result_file = save_test_result("censys", "standalone", result_data, target_filename)
...
class TestCensysLangChain:
    """Test Censys tool with LangChain agent."""
    
    def test_censys_langchain_agent(self):
        """Test censys tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Censys tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Censys to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("censys", "langchain", result_data, target_filename)
...
class TestCensysCrewAI:
    """Test Censys tool with CrewAI agent."""
    
    def test_censys_crewai_agent(self):
        """Test censys tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Censys",
        ...
        task = Task(
            description=f"Use Censys to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Censys tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("censys", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Censys Tool Tests (censys)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCensysStandalone()
        test.test_censys_standalone()
    ...
    try:
        test = TestCensysLangChain()
        test.test_censys_langchain_agent()
    ...
    try:
        test = TestCensysCrewAI()
        test.test_censys_crewai_agent()
