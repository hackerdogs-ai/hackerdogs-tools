"""
Test suite for CyberCrime-Tracker.net tool (cybercrimetracker)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cybercrimetracker_langchain import sfp_cybercrimetracker
from hackerdogs_tools.osint.spiderfoot_modules.sfp_cybercrimetracker_crewai import SfpCybercrimetrackerTool
...
class TestCybercrimetrackerStandalone:
    """Test CyberCrime-Tracker.net tool standalone execution."""
    
    def test_cybercrimetracker_standalone(self):
        """Test cybercrimetracker tool execution without agent."""
        ...
        # Apply checks to sites found to be co-hosted on the target's IP?
        ...
        result = sfp_cybercrimetracker.invoke(invoke_params)
        ...
        result_file = save_test_result("cybercrimetracker", "standalone", result_data, target_filename)
...
class TestCybercrimetrackerLangChain:
    """Test CyberCrime-Tracker.net tool with LangChain agent."""
    
    def test_cybercrimetracker_langchain_agent(self):
        """Test cybercrimetracker tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_cybercrimetracker]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CyberCrime-Tracker.net tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CyberCrime-Tracker.net to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("cybercrimetracker", "langchain", result_data, target_filename)
...
class TestCybercrimetrackerCrewAI:
    """Test CyberCrime-Tracker.net tool with CrewAI agent."""
    
    def test_cybercrimetracker_crewai_agent(self):
        """Test cybercrimetracker tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CyberCrime-Tracker.net",
            ...
            tools=[SfpCybercrimetrackerTool()],
            llm=llm,
            verbose=True
        )
        ...
        task = Task(
            description=f"Use CyberCrime-Tracker.net to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CyberCrime-Tracker.net tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("cybercrimetracker", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CyberCrime-Tracker.net Tool Tests (cybercrimetracker)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCybercrimetrackerStandalone()
        test.test_cybercrimetracker_standalone()
    ...
    try:
        test = TestCybercrimetrackerLangChain()
        test.test_cybercrimetracker_langchain_agent()
    ...
    try:
        test = TestCybercrimetrackerCrewAI()
        test.test_cybercrimetracker_crewai_agent()
