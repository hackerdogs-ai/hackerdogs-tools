"""
Test suite for Whois tool (whois)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_whois_langchain import sfp_whois
from hackerdogs_tools.osint.spiderfoot_modules.sfp_whois_crewai import SfpWhoisTool
...
class TestWhoisStandalone:
    """Test Whois tool standalone execution."""
    
    def test_whois_standalone(self):
        """Test whois tool execution without agent."""
        ...
        result = sfp_whois.invoke(invoke_params)
        ...
        result_file = save_test_result("whois", "standalone", result_data, target_filename)
...
class TestWhoisLangChain:
    """Test Whois tool with LangChain agent."""
    
    def test_whois_langchain_agent(self):
        """Test whois tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Whois tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Whois to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("whois", "langchain", result_data, target_filename)
...
class TestWhoisCrewAI:
    """Test Whois tool with CrewAI agent."""
    
    def test_whois_crewai_agent(self):
        """Test whois tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Whois",
        ...
        task = Task(
            description=f"Use Whois to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Whois tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("whois", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Whois Tool Tests (whois)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestWhoisStandalone()
        test.test_whois_standalone()
    ...
    try:
        test = TestWhoisLangChain()
        test.test_whois_langchain_agent()
    ...
    try:
        test = TestWhoisCrewAI()
        test.test_whois_crewai_agent()
