"""
Test suite for F-Secure Riddler.io tool (fsecure_riddler)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_fsecure_riddler_langchain import sfp_fsecure_riddler
from hackerdogs_tools.osint.spiderfoot_modules.sfp_fsecure_riddler_crewai import SfpFsecureriddlerTool
...
class TestFsecureriddlerStandalone:
    """Test F-Secure Riddler.io tool standalone execution."""
    
    def test_fsecure_riddler_standalone(self):
        """Test fsecure_riddler tool execution without agent."""
        ...
        # Verify host names resolve
        invoke_params["verify"] = True
        # F-Secure Riddler.io username
        invoke_params["username"] = ""
        # F-Secure Riddler.io password
        invoke_params["password"] = ""
        # API key will be retrieved from runtime.state or environment
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_fsecure_riddler.invoke(invoke_params)
        ...
        result_file = save_test_result("fsecure_riddler", "standalone", result_data, target_filename)
...
class TestFsecureriddlerLangChain:
    """Test F-Secure Riddler.io tool with LangChain agent."""
    
    def test_fsecure_riddler_langchain_agent(self):
        """Test fsecure_riddler tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_fsecure_riddler]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the F-Secure Riddler.io tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use F-Secure Riddler.io to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("fsecure_riddler", "langchain", result_data, target_filename)
...
class TestFsecureriddlerCrewAI:
    """Test F-Secure Riddler.io tool with CrewAI agent."""
    
    def test_fsecure_riddler_crewai_agent(self):
        """Test fsecure_riddler tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using F-Secure Riddler.io",
        ...
        task = Task(
            description=f"Use F-Secure Riddler.io to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from F-Secure Riddler.io tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("fsecure_riddler", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running F-Secure Riddler.io Tool Tests (fsecure_riddler)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestFsecureriddlerStandalone()
        test.test_fsecure_riddler_standalone()
    ...
    try:
        test = TestFsecureriddlerLangChain()
        test.test_fsecure_riddler_langchain_agent()
    ...
    try:
        test = TestFsecureriddlerCrewAI()
        test.test_fsecure_riddler_crewai_agent()
