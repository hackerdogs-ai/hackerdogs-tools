"""
Test suite for Bing tool (bingsearch)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bingsearch_langchain import sfp_bingsearch
from hackerdogs_tools.osint.spiderfoot_modules.sfp_bingsearch_crewai import SfpBingsearchTool
...
class TestBingsearchStandalone:
    """Test Bing tool standalone execution."""
    
    def test_bingsearch_standalone(self):
        """Test bingsearch tool execution without agent."""
        ...
        # Number of max bing results to request from the API.
        ...
        result = sfp_bingsearch.invoke(invoke_params)
        ...
        result_file = save_test_result("bingsearch", "standalone", result_data, target_filename)
...
class TestBingsearchLangChain:
    """Test Bing tool with LangChain agent."""
    
    def test_bingsearch_langchain_agent(self):
        """Test bingsearch tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_bingsearch]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Bing tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Bing to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("bingsearch", "langchain", result_data, target_filename)
...
class TestBingsearchCrewAI:
    """Test Bing tool with CrewAI agent."""
    
    def test_bingsearch_crewai_agent(self):
        """Test bingsearch tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Bing",
        ...
        task = Task(
            description=f"Use Bing to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Bing tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("bingsearch", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Bing Tool Tests (bingsearch)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBingsearchStandalone()
        test.test_bingsearch_standalone()
    ...
    try:
        test = TestBingsearchLangChain()
        test.test_bingsearch_langchain_agent()
    ...
    try:
        test = TestBingsearchCrewAI()
        test.test_bingsearch_crewai_agent()
