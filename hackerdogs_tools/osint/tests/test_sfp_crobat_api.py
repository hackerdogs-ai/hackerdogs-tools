"""
Test suite for Crobat API tool (crobat_api)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crobat_api_langchain import sfp_crobat_api
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crobat_api_crewai import SfpCrobatapiTool
...
class TestCrobatapiStandalone:
    """Test Crobat API tool standalone execution."""
    
    def test_crobat_api_standalone(self):
        """Test crobat_api tool execution without agent."""
        ...
        # DNS resolve each identified subdomain.
        invoke_params["verify"] = True
        # Maximum number of pages of results to fetch.
        invoke_params["max_pages"] = 10
        # Delay between requests, in seconds.
        invoke_params["delay"] = 1
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_crobat_api.invoke(invoke_params)
        ...
        result_file = save_test_result("crobat_api", "standalone", result_data, target_filename)
...
class TestCrobatapiLangChain:
    """Test Crobat API tool with LangChain agent."""
    
    def test_crobat_api_langchain_agent(self):
        """Test crobat_api tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_crobat_api]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Crobat API tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Crobat API to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("crobat_api", "langchain", result_data, target_filename)
...
class TestCrobatapiCrewAI:
    """Test Crobat API tool with CrewAI agent."""
    
    def test_crobat_api_crewai_agent(self):
        """Test crobat_api tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Crobat API",
        ...
        task = Task(
            description=f"Use Crobat API to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Crobat API tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("crobat_api", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Crobat API Tool Tests (crobat_api)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCrobatapiStandalone()
        test.test_crobat_api_standalone()
    ...
    try:
        test = TestCrobatapiLangChain()
        test.test_crobat_api_langchain_agent()
    ...
    try:
        test = TestCrobatapiCrewAI()
        test.test_crobat_api_crewai_agent()
