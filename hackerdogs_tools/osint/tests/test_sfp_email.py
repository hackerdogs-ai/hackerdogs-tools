"""
Test suite for E-Mail Address Extractor tool (email)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_email_langchain import sfp_email
from hackerdogs_tools.osint.spiderfoot_modules.sfp_email_crewai import SfpEmailTool
...
class TestEmailStandalone:
    """Test E-Mail Address Extractor tool standalone execution."""
    
    def test_email_standalone(self):
        """Test email tool execution without agent."""
        ...
        result = sfp_email.invoke(invoke_params)
        ...
        result_file = save_test_result("email", "standalone", result_data, target_filename)
...
class TestEmailLangChain:
    """Test E-Mail Address Extractor tool with LangChain agent."""
    
    def test_email_langchain_agent(self):
        """Test email tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the E-Mail Address Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use E-Mail Address Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("email", "langchain", result_data, target_filename)
...
class TestEmailCrewAI:
    """Test E-Mail Address Extractor tool with CrewAI agent."""
    
    def test_email_crewai_agent(self):
        """Test email tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using E-Mail Address Extractor",
        ...
        task = Task(
            description=f"Use E-Mail Address Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from E-Mail Address Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("email", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running E-Mail Address Extractor Tool Tests (email)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestEmailStandalone()
        test.test_email_standalone()
    ...
    try:
        test = TestEmailLangChain()
        test.test_email_langchain_agent()
    ...
    try:
        test = TestEmailCrewAI()
        test.test_email_crewai_agent()
