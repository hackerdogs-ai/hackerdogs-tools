"""
Test suite for Certificate Transparency tool (crt)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crt_langchain import sfp_crt
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crt_crewai import SfpCrtTool
...
class TestCrtStandalone:
    """Test Certificate Transparency tool standalone execution."""
    
    def test_crt_standalone(self):
        """Test crt tool execution without agent."""
        ...
        # Fetch each certificate found, for processing by other modules.
        invoke_params["fetchcerts"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_crt.invoke(invoke_params)
        ...
        result_file = save_test_result("crt", "standalone", result_data, target_filename)
...
class TestCrtLangChain:
    """Test Certificate Transparency tool with LangChain agent."""
    
    def test_crt_langchain_agent(self):
        """Test crt tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Certificate Transparency tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Certificate Transparency to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("crt", "langchain", result_data, target_filename)
...
class TestCrtCrewAI:
    """Test Certificate Transparency tool with CrewAI agent."""
    
    def test_crt_crewai_agent(self):
        """Test crt tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Certificate Transparency",
        ...
        task = Task(
            description=f"Use Certificate Transparency to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Certificate Transparency tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("crt", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Certificate Transparency Tool Tests (crt)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCrtStandalone()
        test.test_crt_standalone()
    ...
    try:
        test = TestCrtLangChain()
        test.test_crt_langchain_agent()
