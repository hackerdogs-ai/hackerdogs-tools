...
class TestSherlockStandalone:
    ...
    def test_sherlock_standalone(self):
        ...
        # Use a random realistic username (sherlock searches for usernames, not domains)
        test_username = get_random_username()
        
        # Tools are StructuredTool objects - use invoke() method
        # Sherlock requires "username" parameter, not "domain"
        ...
        result_file = save_test_result("sherlock", "standalone", result_data, test_username)
...
class TestSherlockLangChain:
    ...
    def test_sherlock_langchain_agent(self, agent):
        """Test sherlock tool with LangChain agent."""
        # Use a test username (sherlock searches for usernames)
        test_username = "testuser123"
        
        # Execute query directly (agent is a runnable in LangChain 1.x)
        # ToolRuntime is automatically injected by the agent
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Search for username {test_username} across social media using Sherlock")]
        })
        ...
        try:
            ...
            result_file = save_test_result("sherlock", "langchain", result_data, test_username)
...
class TestSherlockCrewAI:
    ...
    def test_sherlock_crewai_agent(self, agent, llm):
        ...
        task = Task(
            description=f"Search for username {test_username} across social media using Sherlock",
            agent=agent,
        ...
        try:
            ...
            result_file = save_test_result("sherlock", "crewai", result_data, test_username)
