"""
Test suite for CoinBlocker Lists tool (coinblocker)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_coinblocker_langchain import sfp_coinblocker
from hackerdogs_tools.osint.spiderfoot_modules.sfp_coinblocker_crewai import SfpCoinblockerTool
...
class TestCoinblockerStandalone:
    """Test CoinBlocker Lists tool standalone execution."""
    
    def test_coinblocker_standalone(self):
        """Test coinblocker tool execution without agent."""
        ...
        result = sfp_coinblocker.invoke(invoke_params)
        ...
        result_file = save_test_result("coinblocker", "standalone", result_data, target_filename)
...
class TestCoinblockerLangChain:
    """Test CoinBlocker Lists tool with LangChain agent."""
    
    def test_coinblocker_langchain_agent(self):
        """Test coinblocker tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_coinblocker]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CoinBlocker Lists tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CoinBlocker Lists to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("coinblocker", "langchain", result_data, target_filename)
...
class TestCoinblockerCrewAI:
    """Test CoinBlocker Lists tool with CrewAI agent."""
    
    def test_coinblocker_crewai_agent(self):
        """Test coinblocker tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CoinBlocker Lists",
        ...
        task = Task(
            description=f"Use CoinBlocker Lists to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CoinBlocker Lists tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("coinblocker", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CoinBlocker Lists Tool Tests (coinblocker)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCoinblockerStandalone()
        test.test_coinblocker_standalone()
    ...
    try:
        test = TestCoinblockerLangChain()
        test.test_coinblocker_langchain_agent()
    ...
    try:
        test = TestCoinblockerCrewAI()
        test.test_coinblocker_crewai_agent()
