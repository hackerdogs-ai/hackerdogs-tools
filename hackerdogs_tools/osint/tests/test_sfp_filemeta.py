"""
Test suite for File Metadata Extractor tool (filemeta)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_filemeta_langchain import sfp_filemeta
from hackerdogs_tools.osint.spiderfoot_modules.sfp_filemeta_crewai import SfpFilemetaTool
...
class TestFilemetaStandalone:
    """Test File Metadata Extractor tool standalone execution."""
    
    def test_filemeta_standalone(self):
        """Test filemeta tool execution without agent."""
        ...
        # File extensions of files you want to analyze the meta data of (only PDF, DOCX, XLSX and PPTX are supported.)
        invoke_params["fileexts"] = ['docx', 'pptx', 'pdf', 'jpg', 'jpeg', 'tiff', 'tif']
        # Download timeout for files, in seconds.
        invoke_params["timeout"] = 300
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_filemeta.invoke(invoke_params)
        ...
        result_file = save_test_result("filemeta", "standalone", result_data, target_filename)
...
class TestFilemetaLangChain:
    """Test File Metadata Extractor tool with LangChain agent."""
    
    def test_filemeta_langchain_agent(self):
        """Test filemeta tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the File Metadata Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use File Metadata Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("filemeta", "langchain", result_data, target_filename)
...
class TestFilemetaCrewAI:
    """Test File Metadata Extractor tool with CrewAI agent."""
    
    def test_filemeta_crewai_agent(self):
        """Test filemeta tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using File Metadata Extractor",
        ...
        task = Task(
            description=f"Use File Metadata Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from File Metadata Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("filemeta", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running File Metadata Extractor Tool Tests (filemeta)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestFilemetaStandalone()
        test.test_filemeta_standalone()
    ...
    try:
        test = TestFilemetaLangChain()
        test.test_filemeta_langchain_agent()
    ...
    try:
        test = TestFilemetaCrewAI()
        test.test_filemeta_crewai_agent()
