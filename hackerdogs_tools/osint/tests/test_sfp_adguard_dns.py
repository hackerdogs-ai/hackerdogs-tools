"""
Test suite for AdGuard DNS tool (adguard_dns)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_adguard_dns_langchain import sfp_adguard_dns
from hackerdogs_tools.osint.spiderfoot_modules.sfp_adguard_dns_crewai import SfpAdguarddnsTool
...
class TestAdguarddnsStandalone:
    """Test AdGuard DNS tool standalone execution."""
    
    def test_adguard_dns_standalone(self):
        """Test adguard_dns tool execution without agent."""
        ...
        result = sfp_adguard_dns.invoke(invoke_params)
        ...
        result_file = save_test_result("adguard_dns", "standalone", result_data, target_filename)
...
class TestAdguarddnsLangChain:
    """Test AdGuard DNS tool with LangChain agent."""
    
    def test_adguard_dns_langchain_agent(self):
        """Test adguard_dns tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_adguard_dns]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the AdGuard DNS tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use AdGuard DNS to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("adguard_dns", "langchain", result_data, target_filename)
...
class TestAdguarddnsCrewAI:
    """Test AdGuard DNS tool with CrewAI agent."""
    
    def test_adguard_dns_crewai_agent(self):
        """Test adguard_dns tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using AdGuard DNS",
        ...
        task = Task(
            description=f"Use AdGuard DNS to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from AdGuard DNS tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("adguard_dns", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running AdGuard DNS Tool Tests (adguard_dns)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAdguarddnsStandalone()
        test.test_adguard_dns_standalone()
    ...
    try:
        test = TestAdguarddnsLangChain()
        test.test_adguard_dns_langchain_agent()
    ...
    try:
        test = TestAdguarddnsCrewAI()
        test.test_adguard_dns_crewai_agent()
