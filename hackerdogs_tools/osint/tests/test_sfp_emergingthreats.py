"""
Test suite for Emerging Threats tool (emergingthreats)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emergingthreats_langchain import sfp_emergingthreats
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emergingthreats_crewai import SfpEmergingthreatsTool
...
class TestEmergingthreatsStandalone:
    """Test Emerging Threats tool standalone execution."""
    
    def test_emergingthreats_standalone(self):
        """Test emergingthreats tool execution without agent."""
        ...
        # Apply checks to affiliate IP addresses?
        ...
        result = sfp_emergingthreats.invoke(invoke_params)
        ...
        result_file = save_test_result("emergingthreats", "standalone", result_data, target_filename)
...
class TestEmergingthreatsLangChain:
    """Test Emerging Threats tool with LangChain agent."""
    
    def test_emergingthreats_langchain_agent(self):
        """Test emergingthreats tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_emergingthreats]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Emerging Threats tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Emerging Threats to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("emergingthreats", "langchain", result_data, target_filename)
...
class TestEmergingthreatsCrewAI:
    """Test Emerging Threats tool with CrewAI agent."""
    
    def test_emergingthreats_crewai_agent(self):
        """Test emergingthreats tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Emerging Threats",
        ...
        task = Task(
            description=f"Use Emerging Threats to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Emerging Threats tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("emergingthreats", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Emerging Threats Tool Tests (emergingthreats)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestEmergingthreatsStandalone()
        test.test_emergingthreats_standalone()
    ...
    try:
        test = TestEmergingthreatsLangChain()
        test.test_emergingthreats_langchain_agent()
    ...
    try:
        test = TestEmergingthreatsCrewAI()
        test.test_emergingthreats_crewai_agent()
