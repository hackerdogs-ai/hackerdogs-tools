"""
Test suite for EmailRep tool (emailrep)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emailrep_langchain import sfp_emailrep
from hackerdogs_tools.osint.spiderfoot_modules.sfp_emailrep_crewai import SfpEmailrepTool
...
class TestEmailrepStandalone:
    """Test EmailRep tool standalone execution."""
    
    def test_emailrep_standalone(self):
        """Test emailrep tool execution without agent."""
        ...
        result = sfp_emailrep.invoke(invoke_params)
        ...
        result_file = save_test_result("emailrep", "standalone", result_data, target_filename)
...
class TestEmailrepLangChain:
    """Test EmailRep tool with LangChain agent."""
    
    def test_emailrep_langchain_agent(self):
        """Test emailrep tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the EmailRep tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use EmailRep to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("emailrep", "langchain", result_data, target_filename)
...
class TestEmailrepCrewAI:
    """Test EmailRep tool with CrewAI agent."""
    
    def test_emailrep_crewai_agent(self):
        """Test emailrep tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using EmailRep",
        ...
        task = Task(
            description=f"Use EmailRep to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from EmailRep tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("emailrep", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running EmailRep Tool Tests (emailrep)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestEmailrepStandalone()
        test.test_emailrep_standalone()
    ...
    try:
        test = TestEmailrepLangChain()
        test.test_emailrep_langchain_agent()
    ...
    try:
        test = TestEmailrepCrewAI()
        test.test_emailrep_crewai_agent()
