"""
Test suite for DNS Common SRV tool (dnscommonsrv)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnscommonsrv_langchain import sfp_dnscommonsrv
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnscommonsrv_crewai import SfpDnscommonsrvTool
...
class TestDnscommonsrvStandalone:
    """Test DNS Common SRV tool standalone execution."""
    
    def test_dnscommonsrv_standalone(self):
        """Test dnscommonsrv tool execution without agent."""
        ...
        result = sfp_dnscommonsrv.invoke(invoke_params)
        ...
        result_file = save_test_result("dnscommonsrv", "standalone", result_data, target_filename)
...
class TestDnscommonsrvLangChain:
    """Test DNS Common SRV tool with LangChain agent."""
    
    def test_dnscommonsrv_langchain_agent(self):
        """Test dnscommonsrv tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_dnscommonsrv]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNS Common SRV tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNS Common SRV to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnscommonsrv", "langchain", result_data, target_filename)
...
class TestDnscommonsrvCrewAI:
    """Test DNS Common SRV tool with CrewAI agent."""
    
    def test_dnscommonsrv_crewai_agent(self):
        """Test dnscommonsrv tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNS Common SRV",
        ...
        task = Task(
            description=f"Use DNS Common SRV to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNS Common SRV tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnscommonsrv", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNS Common SRV Tool Tests (dnscommonsrv)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnscommonsrvStandalone()
        test.test_dnscommonsrv_standalone()
    ...
    try:
        test = TestDnscommonsrvLangChain()
        test.test_dnscommonsrv_langchain_agent()
    ...
    try:
        test = TestDnscommonsrvCrewAI()
        test.test_dnscommonsrv_crewai_agent()
