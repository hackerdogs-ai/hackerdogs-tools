"""
Test suite for abuse.ch tool (abusech)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_abusech_langchain import sfp_abusech
from hackerdogs_tools.osint.spiderfoot_modules.sfp_abusech_crewai import SfpAbusechTool
...
class TestAbusechStandalone:
    """Test abuse.ch tool standalone execution."""
    
    def test_abusech_standalone(self):
        """Test abusech tool execution without agent."""
        ...
        # Enable abuse.ch Feodo IP check?
        invoke_params["abusefeodoip"] = True
        # Enable abuse.ch SSL Backlist IP check?
        invoke_params["abusesslblip"] = True
        # Enable abuse.ch URLhaus check?
        invoke_params["abuseurlhaus"] = True
        ...
        result = sfp_abusech.invoke(invoke_params)
        ...
        result_file = save_test_result("abusech", "standalone", result_data, target_filename)
...
class TestAbusechLangChain:
    """Test abuse.ch tool with LangChain agent."""
    
    def test_abusech_langchain_agent(self):
        """Test abusech tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the abuse.ch tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use abuse.ch to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("abusech", "langchain", result_data, target_filename)
...
class TestAbusechCrewAI:
    """Test abuse.ch tool with CrewAI agent."""
    
    def test_abusech_crewai_agent(self):
        """Test abusech tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using abuse.ch",
        ...
        task = Task(
            description=f"Use abuse.ch to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from abuse.ch tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("abusech", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running abuse.ch Tool Tests (abusech)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAbusechStandalone()
        test.test_abusech_standalone()
    ...
    try:
        test = TestAbusechLangChain()
        test.test_abusech_langchain_agent()
    ...
    try:
        test = TestAbusechCrewAI()
        test.test_abusech_crewai_agent()
