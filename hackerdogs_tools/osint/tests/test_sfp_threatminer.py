"""
Test suite for ThreatMiner tool (threatminer)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_threatminer_langchain import sfp_threatminer
from hackerdogs_tools.osint.spiderfoot_modules.sfp_threatminer_crewai import SfpThreatminerTool
...
class TestThreatminerStandalone:
    """Test ThreatMiner tool standalone execution."""
    
    def test_threatminer_standalone(self):
        """Test threatminer tool execution without agent."""
        ...
        result = sfp_threatminer.invoke(invoke_params)
        ...
        result_file = save_test_result("threatminer", "standalone", result_data, target_filename)
...
class TestThreatminerLangChain:
    """Test ThreatMiner tool with LangChain agent."""
    
    def test_threatminer_langchain_agent(self):
        """Test threatminer tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_threatminer]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the ThreatMiner tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use ThreatMiner to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("threatminer", "langchain", result_data, target_filename)
...
class TestThreatminerCrewAI:
    """Test ThreatMiner tool with CrewAI agent."""
    
    def test_threatminer_crewai_agent(self):
        """Test threatminer tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using ThreatMiner",
        ...
        task = Task(
            description=f"Use ThreatMiner to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from ThreatMiner tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("threatminer", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running ThreatMiner Tool Tests (threatminer)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestThreatminerStandalone()
        test.test_threatminer_standalone()
    ...
    try:
        test = TestThreatminerLangChain()
        test.test_threatminer_langchain_agent()
    ...
    try:
        test = TestThreatminerCrewAI()
        test.test_threatminer_crewai_agent()
