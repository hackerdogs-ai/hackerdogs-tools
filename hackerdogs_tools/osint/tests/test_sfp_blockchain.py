"""
Test suite for Blockchain tool (blockchain)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_blockchain_langchain import sfp_blockchain
from hackerdogs_tools.osint.spiderfoot_modules.sfp_blockchain_crewai import SfpBlockchainTool
...
class TestBlockchainStandalone:
    """Test Blockchain tool standalone execution."""
    
    def test_blockchain_standalone(self):
        """Test blockchain tool execution without agent."""
        ...
        result = sfp_blockchain.invoke(invoke_params)
        ...
        result_file = save_test_result("blockchain", "standalone", result_data, target_filename)
...
class TestBlockchainLangChain:
    """Test Blockchain tool with LangChain agent."""
    
    def test_blockchain_langchain_agent(self):
        """Test blockchain tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_blockchain]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Blockchain tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Blockchain to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("blockchain", "langchain", result_data, target_filename)
...
class TestBlockchainCrewAI:
    """Test Blockchain tool with CrewAI agent."""
    
    def test_blockchain_crewai_agent(self):
        """Test blockchain tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Blockchain",
        ...
        task = Task(
            description=f"Use Blockchain to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Blockchain tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("blockchain", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Blockchain Tool Tests (blockchain)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBlockchainStandalone()
        test.test_blockchain_standalone()
    ...
    try:
        test = TestBlockchainLangChain()
        test.test_blockchain_langchain_agent()
    ...
    try:
        test = TestBlockchainCrewAI()
        test.test_blockchain_crewai_agent()
