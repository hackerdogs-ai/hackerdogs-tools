"""
Test suite for CommonCrawl tool (commoncrawl)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_commoncrawl_langchain import sfp_commoncrawl
from hackerdogs_tools.osint.spiderfoot_modules.sfp_commoncrawl_crewai import SfpCommoncrawlTool
...
class TestCommoncrawlStandalone:
    """Test CommonCrawl tool standalone execution."""
    
    def test_commoncrawl_standalone(self):
        """Test commoncrawl tool execution without agent."""
        ...
        # Number of most recent indexes to attempt, because results tend to be occasionally patchy.
        invoke_params["indexes"] = 6
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_commoncrawl.invoke(invoke_params)
        ...
        result_file = save_test_result("commoncrawl", "standalone", result_data, target_filename)
...
class TestCommoncrawlLangChain:
    """Test CommonCrawl tool with LangChain agent."""
    
    def test_commoncrawl_langchain_agent(self):
        """Test commoncrawl tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_commoncrawl]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CommonCrawl tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CommonCrawl to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("commoncrawl", "langchain", result_data, target_filename)
...
class TestCommoncrawlCrewAI:
    """Test CommonCrawl tool with CrewAI agent."""
    
    def test_commoncrawl_crewai_agent(self):
        """Test commoncrawl tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CommonCrawl",
        ...
        task = Task(
            description=f"Use CommonCrawl to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CommonCrawl tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("commoncrawl", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CommonCrawl Tool Tests (commoncrawl)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCommoncrawlStandalone()
        test.test_commoncrawl_standalone()
    ...
    try:
        test = TestCommoncrawlLangChain()
        test.test_commoncrawl_langchain_agent()
    ...
    try:
        test = TestCommoncrawlCrewAI()
        test.test_commoncrawl_crewai_agent()
