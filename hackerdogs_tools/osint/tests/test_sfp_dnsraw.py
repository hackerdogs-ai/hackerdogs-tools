"""
Test suite for DNS Raw Records tool (dnsraw)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsraw_langchain import sfp_dnsraw
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsraw_crewai import SfpDnsrawTool
...
class TestDnsrawStandalone:
    """Test DNS Raw Records tool standalone execution."""
    
    def test_dnsraw_standalone(self):
        """Test dnsraw tool execution without agent."""
        ...
        result = sfp_dnsraw.invoke(invoke_params)
        ...
        result_file = save_test_result("dnsraw", "standalone", result_data, target_filename)
...
class TestDnsrawLangChain:
    """Test DNS Raw Records tool with LangChain agent."""
    
    def test_dnsraw_langchain_agent(self):
        """Test dnsraw tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNS Raw Records tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNS Raw Records to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnsraw", "langchain", result_data, target_filename)
...
class TestDnsrawCrewAI:
    """Test DNS Raw Records tool with CrewAI agent."""
    
    def test_dnsraw_crewai_agent(self):
        """Test dnsraw tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNS Raw Records",
        ...
        task = Task(
            description=f"Use DNS Raw Records to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNS Raw Records tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnsraw", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNS Raw Records Tool Tests (dnsraw)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnsrawStandalone()
        test.test_dnsraw_standalone()
    ...
    try:
        test = TestDnsrawLangChain()
        test.test_dnsraw_langchain_agent()
    ...
    try:
        test = TestDnsrawCrewAI()
        test.test_dnsraw_crewai_agent()
