"""
Test suite for GreyNoise tool (greynoise)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_greynoise_langchain import sfp_greynoise
from hackerdogs_tools.osint.spiderfoot_modules.sfp_greynoise_crewai import SfpGreynoiseTool
...
class TestGreynoiseStandalone:
    """Test GreyNoise tool standalone execution."""
    
    def test_greynoise_standalone(self):
        """Test greynoise tool execution without agent."""
        ...
        invoke_params["age_limit_days"] = 30
        # Look up netblocks deemed to be owned by your target for possible blacklisted hosts on the same target subdomain/domain?
        ...
        # Look up subnets which your target is a part of for blacklisting?
        ...
        result = sfp_greynoise.invoke(invoke_params)
        ...
        result_file = save_test_result("greynoise", "standalone", result_data, target_filename)
...
class TestGreynoiseLangChain:
    """Test GreyNoise tool with LangChain agent."""
    
    def test_greynoise_langchain_agent(self):
        """Test greynoise tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the GreyNoise tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use GreyNoise to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("greynoise", "langchain", result_data, target_filename)
...
class TestGreynoiseCrewAI:
    """Test GreyNoise tool with CrewAI agent."""
    
    def test_greynoise_crewai_agent(self):
        """Test greynoise tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using GreyNoise",
        ...
        task = Task(
            description=f"Use GreyNoise to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from GreyNoise tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("greynoise", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running GreyNoise Tool Tests (greynoise)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestGreynoiseStandalone()
        test.test_greynoise_standalone()
    ...
    try:
        test = TestGreynoiseLangChain()
        test.test_greynoise_langchain_agent()
    ...
    try:
        test = TestGreynoiseCrewAI()
        test.test_greynoise_crewai_agent()
