"""
Test suite for CRXcavator tool (crxcavator)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crxcavator_langchain import sfp_crxcavator
from hackerdogs_tools.osint.spiderfoot_modules.sfp_crxcavator_crewai import SfpCrxcavatorTool
...
class TestCrxcavatorStandalone:
    """Test CRXcavator tool standalone execution."""
    
    def test_crxcavator_standalone(self):
        """Test crxcavator tool execution without agent."""
        ...
        # Verify identified hostnames resolve.
        invoke_params["verify"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_crxcavator.invoke(invoke_params)
        ...
        result_file = save_test_result("crxcavator", "standalone", result_data, target_filename)
...
class TestCrxcavatorLangChain:
    """Test CRXcavator tool with LangChain agent."""
    
    def test_crxcavator_langchain_agent(self):
        """Test crxcavator tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_crxcavator]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CRXcavator tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CRXcavator to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("crxcavator", "langchain", result_data, target_filename)
...
class TestCrxcavatorCrewAI:
    """Test CRXcavator tool with CrewAI agent."""
    
    def test_crxcavator_crewai_agent(self):
        """Test crxcavator tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CRXcavator",
        ...
        task = Task(
            description=f"Use CRXcavator to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CRXcavator tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("crxcavator", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CRXcavator Tool Tests (crxcavator)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCrxcavatorStandalone()
        test.test_crxcavator_standalone()
    ...
    try:
        test = TestCrxcavatorLangChain()
        test.test_crxcavator_langchain_agent()
    ...
    try:
        test = TestCrxcavatorCrewAI()
        test.test_crxcavator_crewai_agent()
