"""
Test suite for Credit Card Number Extractor tool (creditcard)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_creditcard_langchain import sfp_creditcard
from hackerdogs_tools.osint.spiderfoot_modules.sfp_creditcard_crewai import SfpCreditcardTool
...
class TestCreditcardStandalone:
    """Test Credit Card Number Extractor tool standalone execution."""
    
    def test_creditcard_standalone(self):
        """Test creditcard tool execution without agent."""
        ...
        result = sfp_creditcard.invoke(invoke_params)
        ...
        result_file = save_test_result("creditcard", "standalone", result_data, target_filename)
...
class TestCreditcardLangChain:
    """Test Credit Card Number Extractor tool with LangChain agent."""
    
    def test_creditcard_langchain_agent(self):
        """Test creditcard tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_creditcard]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Credit Card Number Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Credit Card Number Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("creditcard", "langchain", result_data, target_filename)
...
class TestCreditcardCrewAI:
    """Test Credit Card Number Extractor tool with CrewAI agent."""
    
    def test_creditcard_crewai_agent(self):
        """Test creditcard tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Credit Card Number Extractor",
        ...
        task = Task(
            description=f"Use Credit Card Number Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Credit Card Number Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("creditcard", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Credit Card Number Extractor Tool Tests (creditcard)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCreditcardStandalone()
        test.test_creditcard_standalone()
    ...
    try:
        test = TestCreditcardLangChain()
        test.test_creditcard_langchain_agent()
    ...
    try:
        test = TestCreditcardCrewAI()
        test.test_creditcard_crewai_agent()
