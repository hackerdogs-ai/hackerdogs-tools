"""
Test suite for CertSpotter tool (certspotter)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_certspotter_langchain import sfp_certspotter
from hackerdogs_tools.osint.spiderfoot_modules.sfp_certspotter_crewai import SfpCertspotterTool
...
class TestCertspotterStandalone:
    """Test CertSpotter tool standalone execution."""
    
    def test_certspotter_standalone(self):
        """Test certspotter tool execution without agent."""
        ...
        # Verify certificate subject alternative names resolve.
        invoke_params["verify"] = True
        # Maximum number of pages of results to fetch.
        invoke_params["max_pages"] = 20
        # Number of days in the future a certificate expires to consider it as expiring.
        invoke_params["certexpiringdays"] = 30
        # API key will be retrieved from runtime.state or environment
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_certspotter.invoke(invoke_params)
        ...
        result_file = save_test_result("certspotter", "standalone", result_data, target_filename)
...
class TestCertspotterLangChain:
    """Test CertSpotter tool with LangChain agent."""
    
    def test_certspotter_langchain_agent(self):
        """Test certspotter tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_certspotter]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the CertSpotter tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use CertSpotter to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("certspotter", "langchain", result_data, target_filename)
...
class TestCertspotterCrewAI:
    """Test CertSpotter tool with CrewAI agent."""
    
    def test_certspotter_crewai_agent(self):
        """Test certspotter tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using CertSpotter",
        ...
        task = Task(
            description=f"Use CertSpotter to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from CertSpotter tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("certspotter", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running CertSpotter Tool Tests (certspotter)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCertspotterStandalone()
        test.test_certspotter_standalone()
    ...
    try:
        test = TestCertspotterLangChain()
        test.test_certspotter_langchain_agent()
    ...
    try:
        test = TestCertspotterCrewAI()
        test.test_certspotter_crewai_agent()
