"""
Test suite for BotScout tool (botscout)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_botscout_langchain import sfp_botscout
from hackerdogs_tools.osint.spiderfoot_modules.sfp_botscout_crewai import SfpBotscoutTool
...
class TestBotscoutStandalone:
    """Test BotScout tool standalone execution."""
    
    def test_botscout_standalone(self):
        """Test botscout tool execution without agent."""
        ...
        result = sfp_botscout.invoke(invoke_params)
        ...
        result_file = save_test_result("botscout", "standalone", result_data, target_filename)
...
class TestBotscoutLangChain:
    """Test BotScout tool with LangChain agent."""
    
    def test_botscout_langchain_agent(self):
        """Test botscout tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the BotScout tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use BotScout to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("botscout", "langchain", result_data, target_filename)
...
class TestBotscoutCrewAI:
    """Test BotScout tool with CrewAI agent."""
    
    def test_botscout_crewai_agent(self):
        """Test botscout tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using BotScout",
        ...
        task = Task(
            description=f"Use BotScout to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from BotScout tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("botscout", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running BotScout Tool Tests (botscout)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBotscoutStandalone()
        test.test_botscout_standalone()
    ...
    try:
        test = TestBotscoutLangChain()
        test.test_botscout_langchain_agent()
    ...
    try:
        test = TestBotscoutCrewAI()
        test.test_botscout_crewai_agent()
