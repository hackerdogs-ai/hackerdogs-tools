"""
Test suite for Country Name Extractor tool (countryname)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_countryname_langchain import sfp_countryname
from hackerdogs_tools.osint.spiderfoot_modules.sfp_countryname_crewai import SfpCountrynameTool
...
class TestCountrynameStandalone:
    """Test Country Name Extractor tool standalone execution."""
    
    def test_countryname_standalone(self):
        """Test countryname tool execution without agent."""
        ...
        # Obtain country name from co-hosted sites
        invoke_params["cohosted"] = True
        # Obtain country name from affiliate sites
        invoke_params["affiliate"] = True
        # Parse TLDs not associated with any country as default country domains
        invoke_params["noncountrytld"] = True
        # Obtain country name from similar domains
        invoke_params["similardomain"] = False
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_countryname.invoke(invoke_params)
        ...
        result_file = save_test_result("countryname", "standalone", result_data, target_filename)
...
class TestCountrynameLangChain:
    """Test Country Name Extractor tool with LangChain agent."""
    
    def test_countryname_langchain_agent(self):
        """Test countryname tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_countryname]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Country Name Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Country Name Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("countryname", "langchain", result_data, target_filename)
...
class TestCountrynameCrewAI:
    """Test Country Name Extractor tool with CrewAI agent."""
    
    def test_countryname_crewai_agent(self):
        """Test countryname tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Country Name Extractor",
        ...
        task = Task(
            description=f"Use Country Name Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Country Name Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("countryname", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Country Name Extractor Tool Tests (countryname)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCountrynameStandalone()
        test.test_countryname_standalone()
    ...
    try:
        test = TestCountrynameLangChain()
        test.test_countryname_langchain_agent()
    ...
    try:
        test = TestCountrynameCrewAI()
        test.test_countryname_crewai_agent()
