"""
Test suite for BuiltWith tool (builtwith)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_builtwith_langchain import sfp_builtwith
from hackerdogs_tools.osint.spiderfoot_modules.sfp_builtwith_crewai import SfpBuiltwithTool
...
class TestBuiltwithStandalone:
    """Test BuiltWith tool standalone execution."""
    
    def test_builtwith_standalone(self):
        """Test builtwith tool execution without agent."""
        ...
        # The maximum age of the data returned, in days, in order to be considered valid.
        invoke_params["maxage"] = 30
        # API key will be retrieved from runtime.state or environment
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_builtwith.invoke(invoke_params)
        ...
        result_file = save_test_result("builtwith", "standalone", result_data, target_filename)
...
class TestBuiltwithLangChain:
    """Test BuiltWith tool with LangChain agent."""
    
    def test_builtwith_langchain_agent(self):
        """Test builtwith tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the BuiltWith tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use BuiltWith to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("builtwith", "langchain", result_data, target_filename)
...
class TestBuiltwithCrewAI:
    """Test BuiltWith tool with CrewAI agent."""
    
    def test_builtwith_crewai_agent(self):
        """Test builtwith tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using BuiltWith",
        ...
        task = Task(
            description=f"Use BuiltWith to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from BuiltWith tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("builtwith", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running BuiltWith Tool Tests (builtwith)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestBuiltwithStandalone()
        test.test_builtwith_standalone()
    ...
    try:
        test = TestBuiltwithLangChain()
        test.test_builtwith_langchain_agent()
    ...
    try:
        test = TestBuiltwithCrewAI()
        test.test_builtwith_crewai_agent()
