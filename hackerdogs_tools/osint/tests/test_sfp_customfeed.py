"""
Test suite for Custom Threat Feed tool (customfeed)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_customfeed_langchain import sfp_customfeed
from hackerdogs_tools.osint.spiderfoot_modules.sfp_customfeed_crewai import SfpCustomfeedTool
...
class TestCustomfeedStandalone:
    """Test Custom Threat Feed tool standalone execution."""
    
    def test_customfeed_standalone(self):
        """Test customfeed tool execution without agent."""
        ...
        # The URL where the feed can be found. Exact matching is performed so the format must be a single line per host, ASN, domain, IP or netblock.
        invoke_params["url"] = ""
        # Maximum age of data in hours before re-downloading. 0 to always download.
        invoke_params["cacheperiod"] = 0
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_customfeed.invoke(invoke_params)
        ...
        result_file = save_test_result("customfeed", "standalone", result_data, target_filename)
...
class TestCustomfeedLangChain:
    """Test Custom Threat Feed tool with LangChain agent."""
    
    def test_customfeed_langchain_agent(self):
        """Test customfeed tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_customfeed]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Custom Threat Feed tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Custom Threat Feed to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("customfeed", "langchain", result_data, target_filename)
...
class TestCustomfeedCrewAI:
    """Test Custom Threat Feed tool with CrewAI agent."""
    
    def test_customfeed_crewai_agent(self):
        """Test customfeed tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Custom Threat Feed",
        ...
        task = Task(
            description=f"Use Custom Threat Feed to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Custom Threat Feed tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("customfeed", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Custom Threat Feed Tool Tests (customfeed)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestCustomfeedStandalone()
        test.test_customfeed_standalone()
    ...
    try:
        test = TestCustomfeedLangChain()
        test.test_customfeed_langchain_agent()
    ...
    try:
        test = TestCustomfeedCrewAI()
        test.test_customfeed_crewai_agent()
