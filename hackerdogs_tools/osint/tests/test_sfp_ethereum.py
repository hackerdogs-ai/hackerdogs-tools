"""
Test suite for Ethereum Address Extractor tool (ethereum)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_ethereum_langchain import sfp_ethereum
from hackerdogs_tools.osint.spiderfoot_modules.sfp_ethereum_crewai import SfpEthereumTool
...
class TestEthereumStandalone:
    """Test Ethereum Address Extractor tool standalone execution."""
    
    def test_ethereum_standalone(self):
        """Test ethereum tool execution without agent."""
        ...
        result = sfp_ethereum.invoke(invoke_params)
        ...
        result_file = save_test_result("ethereum", "standalone", result_data, target_filename)
...
class TestEthereumLangChain:
    """Test Ethereum Address Extractor tool with LangChain agent."""
    
    def test_ethereum_langchain_agent(self):
        """Test ethereum tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Ethereum Address Extractor tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Ethereum Address Extractor to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("ethereum", "langchain", result_data, target_filename)
...
class TestEthereumCrewAI:
    """Test Ethereum Address Extractor tool with CrewAI agent."""
    
    def test_ethereum_crewai_agent(self):
        """Test ethereum tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Ethereum Address Extractor",
        ...
        task = Task(
            description=f"Use Ethereum Address Extractor to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Ethereum Address Extractor tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("ethereum", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Ethereum Address Extractor Tool Tests (ethereum)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestEthereumStandalone()
        test.test_ethereum_standalone()
    ...
    try:
        test = TestEthereumLangChain()
        test.test_ethereum_langchain_agent()
    ...
    try:
        test = TestEthereumCrewAI()
        test.test_ethereum_crewai_agent()
