"""
Test suite for Flickr tool (flickr)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_flickr_langchain import sfp_flickr
from hackerdogs_tools.osint.spiderfoot_modules.sfp_flickr_crewai import SfpFlickrTool
...
class TestFlickrStandalone:
    """Test Flickr tool standalone execution."""
    
    def test_flickr_standalone(self):
        """Test flickr tool execution without agent."""
        ...
        # Number of seconds to pause between fetches.
        invoke_params["pause"] = 1
        # Maximum number of results per page.
        invoke_params["per_page"] = 100
        # Maximum number of pages of results to fetch.
        invoke_params["maxpages"] = 20
        # DNS resolve each identified domain.
        invoke_params["dns_resolve"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_flickr.invoke(invoke_params)
        ...
        result_file = save_test_result("flickr", "standalone", result_data, target_filename)
...
class TestFlickrLangChain:
    """Test Flickr tool with LangChain agent."""
    
    def test_flickr_langchain_agent(self):
        """Test flickr tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Flickr tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Flickr to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("flickr", "langchain", result_data, target_filename)
...
class TestFlickrCrewAI:
    """Test Flickr tool with CrewAI agent."""
    
    def test_flickr_crewai_agent(self):
        """Test flickr tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Flickr",
        ...
        task = Task(
            description=f"Use Flickr to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Flickr tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("flickr", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Flickr Tool Tests (flickr)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestFlickrStandalone()
        test.test_flickr_standalone()
    ...
    try:
        test = TestFlickrLangChain()
        test.test_flickr_langchain_agent()
    ...
    try:
        test = TestFlickrCrewAI()
        test.test_flickr_crewai_agent()
