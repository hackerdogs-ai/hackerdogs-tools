"""
Test suite for DuckDuckGo tool (duckduckgo)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_duckduckgo_langchain import sfp_duckduckgo
from hackerdogs_tools.osint.spiderfoot_modules.sfp_duckduckgo_crewai import SfpDuckduckgoTool
...
class TestDuckduckgoStandalone:
    """Test DuckDuckGo tool standalone execution."""
    
    def test_duckduckgo_standalone(self):
        """Test duckduckgo tool execution without agent."""
        ...
        # For affiliates, look up the domain name, not the hostname. This will usually return more meaningful information about the affiliate.
        invoke_params["affiliatedomains"] = True
        
        # Tools are StructuredTool objects - use invoke() method
        result = sfp_duckduckgo.invoke(invoke_params)
        ...
        result_file = save_test_result("duckduckgo", "standalone", result_data, target_filename)
...
class TestDuckduckgoLangChain:
    """Test DuckDuckGo tool with LangChain agent."""
    
    def test_duckduckgo_langchain_agent(self):
        """Test duckduckgo tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_duckduckgo]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DuckDuckGo tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DuckDuckGo to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("duckduckgo", "langchain", result_data, target_filename)
...
class TestDuckduckgoCrewAI:
    """Test DuckDuckGo tool with CrewAI agent."""
    
    def test_duckduckgo_crewai_agent(self):
        """Test duckduckgo tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DuckDuckGo",
        ...
        task = Task(
            description=f"Use DuckDuckGo to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DuckDuckGo tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("duckduckgo", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DuckDuckGo Tool Tests (duckduckgo)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDuckduckgoStandalone()
        test.test_duckduckgo_standalone()
    ...
    try:
        test = TestDuckduckgoLangChain()
        test.test_duckduckgo_langchain_agent()
    ...
    try:
        test = TestDuckduckgoCrewAI()
        test.test_duckduckgo_crewai_agent()
