"""
Test suite for Etherscan tool (etherscan)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_etherscan_langchain import sfp_etherscan
from hackerdogs_tools.osint.spiderfoot_modules.sfp_etherscan_crewai import SfpEtherscanTool
...
class TestEtherscanStandalone:
    """Test Etherscan tool standalone execution."""
    
    def test_etherscan_standalone(self):
        """Test etherscan tool execution without agent."""
        ...
        result = sfp_etherscan.invoke(invoke_params)
        ...
        result_file = save_test_result("etherscan", "standalone", result_data, target_filename)
...
class TestEtherscanLangChain:
    """Test Etherscan tool with LangChain agent."""
    
    def test_etherscan_langchain_agent(self):
        """Test etherscan tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Etherscan tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Etherscan to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("etherscan", "langchain", result_data, target_filename)
...
class TestEtherscanCrewAI:
    """Test Etherscan tool with CrewAI agent."""
    
    def test_etherscan_crewai_agent(self):
        """Test etherscan tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Etherscan",
        ...
        task = Task(
            description=f"Use Etherscan to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Etherscan tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("etherscan", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Etherscan Tool Tests (etherscan)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestEtherscanStandalone()
        test.test_etherscan_standalone()
    ...
    try:
        test = TestEtherscanLangChain()
        test.test_etherscan_langchain_agent()
    ...
    try:
        test = TestEtherscanCrewAI()
        test.test_etherscan_crewai_agent()
