...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_c99_langchain import sfp_c99
from hackerdogs_tools.osint.spiderfoot_modules.sfp_c99_crewai import SfpC99Tool
...
class TestC99Standalone:
    """Test C99 tool standalone execution."""
    
    def test_c99_standalone(self):
        """Test c99 tool execution without agent."""
        ...
        # Verify identified domains still resolve to the associated specified IP address.
        ...
        result = sfp_c99.invoke(invoke_params)
        ...
        result_file = save_test_result("c99", "standalone", result_data, target_filename)
...
class TestC99LangChain:
    """Test C99 tool with LangChain agent."""
    
    def test_c99_langchain_agent(self):
        """Test c99 tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the C99 tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use C99 to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("c99", "langchain", result_data, target_filename)
...
class TestC99CrewAI:
    """Test C99 tool with CrewAI agent."""
    
    def test_c99_crewai_agent(self):
        """Test c99 tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using C99",
        ...
        task = Task(
            description=f"Use C99 to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from C99 tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("c99", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running C99 Tool Tests (c99)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestC99Standalone()
        test.test_c99_standalone()
    ...
    try:
        test = TestC99LangChain()
        test.test_c99_langchain_agent()
