"""
Test suite for DNS Look-aside tool (dnsneighbor)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsneighbor_langchain import sfp_dnsneighbor
from hackerdogs_tools.osint.spiderfoot_modules.sfp_dnsneighbor_crewai import SfpDnsneighborTool
...
class TestDnsneighborStandalone:
    """Test DNS Look-aside tool standalone execution."""
    
    def test_dnsneighbor_standalone(self):
        """Test dnsneighbor tool execution without agent."""
        ...
        # If look-aside is enabled, the netmask size (in CIDR notation) to check. Default is 4 bits (16 hosts).
        invoke_params["lookasidebits"] = 4
        ...
        result = sfp_dnsneighbor.invoke(invoke_params)
        ...
        result_file = save_test_result("dnsneighbor", "standalone", result_data, target_filename)
...
class TestDnsneighborLangChain:
    """Test DNS Look-aside tool with LangChain agent."""
    
    def test_dnsneighbor_langchain_agent(self):
        """Test dnsneighbor tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_dnsneighbor]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the DNS Look-aside tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use DNS Look-aside to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("dnsneighbor", "langchain", result_data, target_filename)
...
class TestDnsneighborCrewAI:
    """Test DNS Look-aside tool with CrewAI agent."""
    
    def test_dnsneighbor_crewai_agent(self):
        """Test dnsneighbor tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using DNS Look-aside",
        ...
        task = Task(
            description=f"Use DNS Look-aside to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from DNS Look-aside tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("dnsneighbor", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running DNS Look-aside Tool Tests (dnsneighbor)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDnsneighborStandalone()
        test.test_dnsneighbor_standalone()
    ...
    try:
        test = TestDnsneighborLangChain()
        test.test_dnsneighbor_langchain_agent()
    ...
    try:
        test = TestDnsneighborCrewAI()
        test.test_dnsneighbor_crewai_agent()
