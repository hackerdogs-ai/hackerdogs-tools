"""
Test suite for Abusix Mail Intelligence tool (abusix)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_abusix_langchain import sfp_abusix
from hackerdogs_tools.osint.spiderfoot_modules.sfp_abusix_crewai import SfpAbusixTool
...
class TestAbusixStandalone:
    """Test Abusix Mail Intelligence tool standalone execution."""
    
    def test_abusix_standalone(self):
        """Test abusix tool execution without agent."""
        ...
        result = sfp_abusix.invoke(invoke_params)
        ...
        result_file = save_test_result("abusix", "standalone", result_data, target_filename)
...
class TestAbusixLangChain:
    """Test Abusix Mail Intelligence tool with LangChain agent."""
    
    def test_abusix_langchain_agent(self):
        """Test abusix tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Abusix Mail Intelligence tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Abusix Mail Intelligence to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("abusix", "langchain", result_data, target_filename)
...
class TestAbusixCrewAI:
    """Test Abusix Mail Intelligence tool with CrewAI agent."""
    
    def test_abusix_crewai_agent(self):
        """Test abusix tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Abusix Mail Intelligence",
        ...
        task = Task(
            description=f"Use Abusix Mail Intelligence to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Abusix Mail Intelligence tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("abusix", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Abusix Mail Intelligence Tool Tests (abusix)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestAbusixStandalone()
        test.test_abusix_standalone()
    ...
    try:
        test = TestAbusixLangChain()
        test.test_abusix_langchain_agent()
    ...
    try:
        test = TestAbusixCrewAI()
        test.test_abusix_crewai_agent()
