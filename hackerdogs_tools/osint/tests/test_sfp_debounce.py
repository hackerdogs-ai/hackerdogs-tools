"""
Test suite for Debounce tool (debounce)
...
from hackerdogs_tools.osint.spiderfoot_modules.sfp_debounce_langchain import sfp_debounce
from hackerdogs_tools.osint.spiderfoot_modules.sfp_debounce_crewai import SfpDebounceTool
...
class TestDebounceStandalone:
    """Test Debounce tool standalone execution."""
    
    def test_debounce_standalone(self):
        """Test debounce tool execution without agent."""
        ...
        result = sfp_debounce.invoke(invoke_params)
        ...
        result_file = save_test_result("debounce", "standalone", result_data, target_filename)
...
class TestDebounceLangChain:
    """Test Debounce tool with LangChain agent."""
    
    def test_debounce_langchain_agent(self):
        """Test debounce tool with LangChain agent."""
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the Debounce tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use Debounce to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("debounce", "langchain", result_data, target_filename)
...
class TestDebounceCrewAI:
    """Test Debounce tool with CrewAI agent."""
    
    def test_debounce_crewai_agent(self):
        """Test debounce tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using Debounce",
        ...
        task = Task(
            description=f"Use Debounce to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from Debounce tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("debounce", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running Debounce Tool Tests (debounce)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestDebounceStandalone()
        test.test_debounce_standalone()
    ...
    try:
        test = TestDebounceLangChain()
        test.test_debounce_langchain_agent()
    ...
    try:
        test = TestDebounceCrewAI()
        test.test_debounce_crewai_agent()
