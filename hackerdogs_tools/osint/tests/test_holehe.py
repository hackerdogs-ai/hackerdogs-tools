...
class TestHoleheStandalone:
    ...
    def test_holehe_standalone(self):
        ...
        result = holehe_search(
            runtime=runtime,
            email=f"test@{get_random_domain()}", only_used=True
        )
...
class TestHoleheLangChain:
    ...
    def agent(self, llm):
        ...
        agent = create_agent(
            llm=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the holehe tool for OSINT operations."
        )
        return agent
    ...
    def test_holehe_langchain_agent(self, agent):
        ...
        result = executor.invoke({
            "messages": [HumanMessage(content=f"Check if email test@{test_domain} is registered on sites using Holehe")],
            "runtime": runtime
        })
...
class TestHoleheCrewAI:
    ...
    def test_holehe_crewai_agent(self, agent, llm):
        """Test holehe tool with CrewAI agent."""
        task = Task(
            description=f"Check if email test@{test_domain} is registered on sites using Holehe",
            agent=agent,
            expected_output="Results from holehe tool"
        )
...
def run_all_tests():
    ...
    try:
        ...
        test = TestHoleheLangChain()
        test.test_holehe_langchain_agent(test.agent(llm))
    ...
    try:
        ...
        test = TestHoleheCrewAI()
        test.test_holehe_crewai_agent(test.agent(llm), llm)
