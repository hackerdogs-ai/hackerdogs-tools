"""
SHODAN Tool for LangChain Agents
...
class SfpShodanSecurityAgentState(AgentState):
    """Extended agent state for SHODAN operations."""
    user_id: str = ""
...
def sfp_shodan(
    ...
    **kwargs: Any
    ...
    Args:
        ...
        target: Target to investigate (IP_ADDRESS, NETBLOCK_OWNER, DOMAIN_NAME, WEB_ANALYTICS_ID)
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_shodan] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_shodan] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_shodan] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
