"""
Cookie Extractor Tool for LangChain Agents
...
class SfpCookieSecurityAgentState(AgentState):
    """Extended agent state for Cookie Extractor operations."""
    user_id: str = ""
...
def sfp_cookie(
    ...
    **kwargs: Any
    ...
    Args:
        ...
        target: Target to investigate (WEBSERVER_HTTPHEADERS)
    
    Returns:
        JSON string with results.
    """
    try:
        safe_log_info(logger, f"[sfp_cookie] Starting", target=target)
        ...
        safe_log_info(logger, f"[sfp_cookie] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_cookie] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
