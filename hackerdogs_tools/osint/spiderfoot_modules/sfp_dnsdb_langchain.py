"""
DNSDB Tool for LangChain Agents
...
class SfpDnsdbSecurityAgentState(AgentState):
    """Extended agent state for DNSDB operations."""
    user_id: str = ""
...
def sfp_dnsdb(
    ...
    **kwargs: Any
    ...
    Args:
        ...
        target: Target to investigate (IP_ADDRESS, IPV6_ADDRESS, DOMAIN_NAME)
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_dnsdb] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_dnsdb] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_dnsdb] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
