"""
GreyNoise Tool for LangChain Agents
...
class SfpGreynoiseSecurityAgentState(AgentState):
    """Extended agent state for GreyNoise operations."""
    user_id: str = ""
...
def sfp_greynoise(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_greynoise] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_greynoise] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_greynoise] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
