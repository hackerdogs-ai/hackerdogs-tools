"""
Leak-Lookup Tool for LangChain Agents
...
class SfpCitadelSecurityAgentState(AgentState):
    """Extended agent state for Leak-Lookup operations."""
    user_id: str = ""
...
def sfp_citadel(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_citadel] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_citadel] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_citadel] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
