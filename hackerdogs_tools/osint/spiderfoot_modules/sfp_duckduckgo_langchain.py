"""
DuckDuckGo Tool for LangChain Agents
...
class SfpDuckduckgoSecurityAgentState(AgentState):
    """Extended agent state for DuckDuckGo operations."""
    user_id: str = ""
...
def sfp_duckduckgo(
    ...
    **kwargs: Any
    ...
    Args:
        ...
        target: Target to investigate (DOMAIN_NAME, DOMAIN_NAME_PARENT, INTERNET_NAME, AFFILIATE_INTERNET_NAME)
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_duckduckgo] Starting", target=target)
        ...
        safe_log_info(logger, f"[sfp_duckduckgo] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_duckduckgo] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
