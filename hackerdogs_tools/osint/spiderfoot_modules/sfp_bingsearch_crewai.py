...
class SfpBingsearchToolSchema(BaseModel):
    """Input schema for BingTool."""
    ...
    api_key: Optional[str] = Field(default=None, description="Bing API Key for Bing search.")


class SfpBingsearchTool(BaseTool):
    """Tool for Obtain information from bing to identify sub-domains and links.."""
    ...
    args_schema: type[BaseModel] = SfpBingsearchToolSchema
    ...
    def _run(
        ...
        **kwargs: Any
        ...
        """Execute Bing."""
        try:
            safe_log_info(logger, f"[SfpBingsearchTool] Starting", target=target, has_api_key=bool(api_key))
            ...
            safe_log_info(logger, f"[SfpBingsearchTool] Complete", target=target, user_id=user_id)
            return json.dumps(result, indent=2)
            
        except Exception as e:
            target = kwargs.get("target", "unknown")
            user_id = kwargs.get("user_id", "")
            safe_log_error(logger, f"[SfpBingsearchTool] Error: {str(e)}", target=target, user_id=user_id, exc_info=True)
            return json.dumps({
                "status": "error",
