"""
ARIN Tool for LangChain Agents
...
class SfpArinSecurityAgentState(AgentState):
    """Extended agent state for ARIN operations."""
    user_id: str = ""
...
def sfp_arin(
    ...
    **kwargs: Any
    ...
    Args:
        ...
        target: Target to investigate (DOMAIN_NAME, HUMAN_NAME)
    
    Returns:
        JSON string with results.
    """
    try:
        safe_log_info(logger, f"[sfp_arin] Starting", target=target)
        ...
        safe_log_info(logger, f"[sfp_arin] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_arin] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
