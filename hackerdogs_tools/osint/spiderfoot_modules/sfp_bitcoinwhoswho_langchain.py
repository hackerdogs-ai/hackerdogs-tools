"""
Bitcoin Who's Who Tool for LangChain Agents
...
class SfpBitcoinwhoswhoSecurityAgentState(AgentState):
    """Extended agent state for Bitcoin Who's Who operations."""
    user_id: str = ""
...
def sfp_bitcoinwhoswho(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_bitcoinwhoswho] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_bitcoinwhoswho] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_bitcoinwhoswho] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
