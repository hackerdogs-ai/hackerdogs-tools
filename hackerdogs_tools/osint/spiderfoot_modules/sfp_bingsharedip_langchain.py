"""
Bing (Shared IPs) Tool for LangChain Agents
...
class SfpBingsharedipSecurityAgentState(AgentState):
    """Extended agent state for Bing (Shared IPs) operations."""
    user_id: str = ""
...
def sfp_bingsharedip(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_bingsharedip] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_bingsharedip] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_bingsharedip] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
