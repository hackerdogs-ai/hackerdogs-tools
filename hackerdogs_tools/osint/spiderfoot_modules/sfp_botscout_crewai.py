"""
BotScout Tool for CrewAI Agents
...
class SfpBotscoutToolSchema(BaseModel):
    """Input schema for BotScoutTool."""
    target: str = Field(..., description="Target to investigate (IP_ADDRESS, EMAILADDR)")
    api_key: Optional[str] = Field(default=None, description="Botscout.com API key. Without this you will be limited to 100 look-ups per day.")


class SfpBotscoutTool(BaseTool):
    """Tool for Searches BotScout.com's database of spam-bot IP addresses and e-mail addresses.."""
    ...
    args_schema: type[BaseModel] = SfpBotscoutToolSchema
    ...
    def _run(
        ...
        **kwargs: Any
        ...
        """Execute BotScout."""
        try:
            safe_log_info(logger, f"[SfpBotscoutTool] Starting", target=target, has_api_key=bool(api_key))
            ...
            safe_log_info(logger, f"[SfpBotscoutTool] Complete", target=target, user_id=user_id)
            return json.dumps(result, indent=2)
            
        except Exception as e:
            target = kwargs.get("target", "unknown")
            user_id = kwargs.get("user_id", "")
            safe_log_error(logger, f"[SfpBotscoutTool] Error: {str(e)}", target=target, user_id=user_id, exc_info=True)
            return json.dumps({
                "status": "error",
