"""
BuiltWith Tool for LangChain Agents
...
class SfpBuiltwithSecurityAgentState(AgentState):
    """Extended agent state for BuiltWith operations."""
    user_id: str = ""
...
def sfp_builtwith(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_builtwith] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_builtwith] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_builtwith] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
