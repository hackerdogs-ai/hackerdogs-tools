"""
File Metadata Extractor Tool for LangChain Agents
...
class SfpFilemetaSecurityAgentState(AgentState):
    """Extended agent state for File Metadata Extractor operations."""
    user_id: str = ""
...
def sfp_filemeta(
    ...
    **kwargs: Any
    ...
    Args:
        ...
        target: Target to investigate (LINKED_URL_INTERNAL, INTERESTING_FILE)
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_filemeta] Starting", target=target)
        ...
        safe_log_info(logger, f"[sfp_filemeta] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_filemeta] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
