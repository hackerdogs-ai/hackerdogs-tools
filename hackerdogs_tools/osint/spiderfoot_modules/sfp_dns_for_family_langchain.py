"""
DNS for Family Tool for LangChain Agents
...
class SfpDnsforfamilySecurityAgentState(AgentState):
    """Extended agent state for DNS for Family operations."""
    user_id: str = ""
...
def sfp_dns_for_family(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_dns_for_family] Starting", target=target)
        ...
        safe_log_info(logger, f"[sfp_dns_for_family] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_dns_for_family] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
