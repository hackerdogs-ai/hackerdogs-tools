"""
BitcoinAbuse Tool for LangChain Agents
...
class SfpBitcoinabuseSecurityAgentState(AgentState):
    """Extended agent state for BitcoinAbuse operations."""
    user_id: str = ""
...
def sfp_bitcoinabuse(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_bitcoinabuse] Starting", target=target, has_api_key=bool(api_key))
        ...
        safe_log_info(logger, f"[sfp_bitcoinabuse] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_bitcoinabuse] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
