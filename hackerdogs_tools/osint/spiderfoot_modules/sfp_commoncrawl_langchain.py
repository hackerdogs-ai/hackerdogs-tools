"""
CommonCrawl Tool for LangChain Agents
...
class SfpCommoncrawlSecurityAgentState(AgentState):
    """Extended agent state for CommonCrawl operations."""
    user_id: str = ""
...
def sfp_commoncrawl(
    ...
    **kwargs: Any
    ...
    """
    try:
        safe_log_info(logger, f"[sfp_commoncrawl] Starting", target=target)
        ...
        safe_log_info(logger, f"[sfp_commoncrawl] Complete", target=target, user_id=user_id)
        ...
        safe_log_error(logger, f"[sfp_commoncrawl] Error: {str(e)}", target=error_target, user_id=error_user_id, exc_info=True)
        return json.dumps({
            "status": "error",
