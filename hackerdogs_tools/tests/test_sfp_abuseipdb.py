"""
Test suite for AbuseIPDB tool (abuseipdb)
...
from hackerdogs_tools.osint.spiderfoot_modules.abuseipdb_langchain import sfp_abuseipdb
from hackerdogs_tools.osint.spiderfoot_modules.abuseipdb_crewai import SfpAbuseipdbTool
...
class TestAbuseipdbStandalone:
    ...
    def test_abuseipdb_standalone(self):
        ...
        result_file = save_test_result("abuseipdb", "standalone", result_data, target_filename)
...
class TestAbuseipdbLangChain:
    ...
    def test_abuseipdb_langchain_agent(self):
        ...
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the AbuseIPDB tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use AbuseIPDB to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("abuseipdb", "langchain", result_data, target_filename)
...
class TestAbuseipdbCrewAI:
    ...
    def test_abuseipdb_crewai_agent(self):
        ...
        task = Task(
            description=f"Use AbuseIPDB to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from AbuseIPDB tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("abuseipdb", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running AbuseIPDB Tool Tests (abuseipdb)")
    print("=" * 80)
    ...
    try:
        ...
        test.test_abuseipdb_langchain_agent()
    ...
    try:
        ...
        test.test_abuseipdb_crewai_agent()
