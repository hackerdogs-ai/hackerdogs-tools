"""
Test suite for VirusTotal tool (virustotal)
...
from hackerdogs_tools.osint.spiderfoot_modules.virustotal_langchain import sfp_virustotal
from hackerdogs_tools.osint.spiderfoot_modules.virustotal_crewai import SfpVirustotalTool
from hackerdogs_tools.osint.tests.test_utils import get_llm_from_env, get_crewai_llm_from_env
from hackerdogs_tools.osint.test_domains import get_random_domain
from hackerdogs_tools.osint.test_identity_data import get_random_ip
...
class TestVirustotalStandalone:
    """Test VirusTotal tool standalone execution."""
    
    def test_virustotal_standalone(self):
        """Test virustotal tool execution without agent."""
        ...
        test_target = "192.168.5.1"  # Use local IP for testing
        ...
        # Are you using a public key? If so SpiderFoot will pause for 15 seconds after each query to avoid VirusTotal dropping requests.
        invoke_params["publicapi"] = True
        # Check co-hosted sites?
        invoke_params["checkcohosts"] = True
        # Check affiliates?
        invoke_params["checkaffiliates"] = True
        # Look up all IPs on netblocks deemed to be owned by your target for possible hosts on the same target subdomain/domain?
        ...
        result = sfp_virustotal.invoke(invoke_params)
        ...
        result_file = save_test_result("virustotal", "standalone", result_data, target_filename)
...
class TestVirustotalLangChain:
    """Test VirusTotal tool with LangChain agent."""
    
    def test_virustotal_langchain_agent(self):
        """Test virustotal tool with LangChain agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_llm_from_env()
        tools = [sfp_virustotal]
        agent = create_agent(
            model=llm,
            tools=tools,
            system_prompt="You are a cybersecurity analyst. Use the VirusTotal tool for OSINT operations. IMPORTANT: Return the tool output verbatim as JSON without any additional commentary or summarization."
        )
        ...
        result = agent.invoke({
            "messages": [HumanMessage(content=f"Use VirusTotal to investigate {test_target}")],
            "user_id": "test_user"
        })
        ...
        if tool_output_message and tool_output_message.content:
            try:
                ...
                target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
                result_file = save_test_result("virustotal", "langchain", result_data, target_filename)
...
class TestVirustotalCrewAI:
    """Test VirusTotal tool with CrewAI agent."""
    
    def test_virustotal_crewai_agent(self):
        """Test virustotal tool with CrewAI agent."""
        # Create agent directly (not using pytest fixtures)
        llm = get_crewai_llm_from_env()
        agent = Agent(
            role="OSINT Analyst",
            goal="Perform OSINT operations using VirusTotal",
        ...
        task = Task(
            description=f"Use VirusTotal to investigate {test_target}. IMPORTANT: Return the tool output verbatim as complete JSON without any additional commentary, summarization, or interpretation.",
            agent=agent,
            expected_output="Complete raw JSON output from VirusTotal tool with status, module, target, raw_response, user_id, and note fields."
        )
        ...
        try:
            target_filename = test_target.replace("@", "_at_").replace(".", "_").replace("/", "_")
            result_file = save_test_result("virustotal", "crewai", result_data, target_filename)
...
def run_all_tests():
    ...
    print("=" * 80)
    print(f"Running VirusTotal Tool Tests (virustotal)")
    print("=" * 80)
    
    # Test 1: Standalone
    print("\n1. Testing Standalone Execution...")
    try:
        test = TestVirustotalStandalone()
        test.test_virustotal_standalone()
    ...
    try:
        test = TestVirustotalLangChain()
        test.test_virustotal_langchain_agent()
    ...
    try:
        test = TestVirustotalCrewAI()
        test.test_virustotal_crewai_agent()
